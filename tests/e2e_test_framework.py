...
class E2ETestFramework:
    ...
    def make_http_request(
        self, url: str, method: str = "GET", timeout: int = 5, data: dict | None = None
    ) -> tuple[int, float]:
        ...
        for test_name, url, method, data in api_tests:
            ...
            try:
                ...
                if method == "GET":
                    ...
                    status_code, response_time = self.make_http_request(
                        url, "POST", data=data
                    )
                    if status_code < 400:
                        results["passed"] += 1
                ...
                results["failed"] += 1
                results["details"].append(
                    {"test": f"api_{test_name}", "status": "failed", "error": str(e)}
                )
    ...
    async def run_frontend_tests(self) -> dict[str, Any]:
        ...
        try:
            ...
            if exists and has_files:
                results["passed"] += 1
                results["details"].append(
                    {"test": "frontend_build_exists", "status": "passed"}
                )
            else:
                results["failed"] += 1
                results["details"].append(
            ...
            results["failed"] += 1
            results["details"].append(
                {"test": "frontend_build_exists", "status": "failed", "error": str(e)}
            )
        ...
        try:
            ...
            if status_code == 200:
                results["passed"] += 1
                results["details"].append(
                    {"test": "frontend_static_serving", "status": "passed"}
                )
            else:
                results["failed"] += 1
                results["details"].append(
            ...
            results["failed"] += 1
            results["details"].append(
                {"test": "frontend_static_serving", "status": "failed", "error": str(e)}
            )
        ...
        for test_name, url, is_rate_limit_test in security_tests:
            ...
            try:
                if is_rate_limit_test:
                    ...
                    status_code, _response_time = self.make_http_request(url)
    ...
    async def run_performance_tests(self) -> dict[str, Any]:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=8, thread_name_prefix="http_worker"
            ) as executor:
    ...
    async def run_ai_service_tests(self) -> dict[str, Any]:
        ...
        ai_tests = [
            ...
            (
                "performance_ai_metrics",
                f"{self.base_url}/api/v1/ai/performance",
                "GET",
                None,
            ),
            (
                "ai_analyze_available",
        ...
        for test_name, url, method, data in ai_tests:
            ...
            try:
                ...
                results["failed"] += 1
                results["details"].append(
                    {"test": f"ai_{test_name}", "status": "failed", "error": str(e)}
                )
    ...
    async def run_all_tests(self) -> dict[str, Any]:
        ...
        for suite_name, test_coro in test_suites:
            ...
            try:
                ...
                all_results["test_suites"].append(
                    {"name": suite_name, "results": result}
                )
                ...
                print(f"❌ {suite_name}: Failed to run - {e!s}")
                all_results["test_suites"].append({"name": suite_name, "error": str(e)})
        ...
        print(
            f"🎯 E2E Test Suite Complete: {all_results['total_passed']}/{all_results['total_tests']} tests passed ({all_results.get('success_rate', 0)}%)"
        )

        return all_results

    def save_results(
        self, results: dict[str, Any], output_file: str = "e2e_test_results.json"
    ):
