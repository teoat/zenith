...
try:
    ...
    logger.warning(
        f"Alembic/SQLAlchemy not available: {e}. Migration testing will be limited."
    )
...
class DatabaseMigrationTester:
    ...
    def test_migration_up(self, db_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            with create_engine(db_url).connect() as conn:
                ...
                if result["success"]:
                    logger.info(
                        f"✅ Migration upgrade successful to revision {current_version}"
                    )
                else:
                    result["errors"].append(
                        f"Expected revision {head_revision}, got {current_version}"
                    )

        except Exception as e:
            result["errors"].append(f"Migration upgrade failed: {e!s}")
    ...
    def test_migration_down(self, db_url: str) -> dict[str, Any]:
        ...
        try:
            ...
            with create_engine(db_url).connect() as conn:
                ...
                if result["success"]:
                    logger.info(
                        f"✅ Migration downgrade successful to revision {current_version}"
                    )
                else:
                    result["errors"].append(
                        f"Expected base revision {base_revision}, got {current_version}"
                    )

        except Exception as e:
            result["errors"].append(f"Migration downgrade failed: {e!s}")
                        ...
                        if table_exists:
                            ...
                            for expected_col in expected_columns:
                                if expected_col in columns:
                                    result["data_checks"].append(
                                        f"✅ Column {expected_col} exists in {table_name}"
                                    )
                                else:
                                    result["data_checks"].append(
                                        f"❌ Column {expected_col} missing in {table_name}"
                                    )
                                    result["success"] = False
                        else:
                            result["data_checks"].append(
                                f"⚠️ Table {table_name} does not exist (may be expected)"
                            )

                    except Exception as e:
                        result["errors"].append(
                            f"Data integrity check failed for {table_name}: {e!s}"
                        )
                        result["success"] = False

        except Exception as e:
            result["errors"].append(f"Data integrity testing failed: {e!s}")
            result["success"] = False
        ...
        try:
            ...
            for run_num in range(result["runs_attempted"]):
                try:
                    ...
                    result["errors"].append(
                        f"Idempotency run {run_num + 1} failed: {e!s}"
                    )
                    result["success"] = False
                    break

        except Exception as e:
            result["errors"].append(f"Idempotency testing failed: {e!s}")
            result["success"] = False
    ...
    def generate_migration_report(self, test_results: dict[str, Any]) -> dict[str, Any]:
        ...
        if not integrity_test.get("success", False):
            report["recommendations"].append(
                "Address data integrity issues in migrations"
            )
        ...
        if all_passed:
            report["recommendations"].append(
                "Migration testing passed - safe for deployment"
            )
            report["recommendations"].append(
...
def main():
    ...
    try:
        ...
        print(
            f"Overall Status: {'✅ PASS' if report.get('overall_success', False) else '❌ FAIL'}"
        )
