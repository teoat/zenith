...
class LoadTester:
    ...
    async def make_request(
        self, session: aiohttp.ClientSession, endpoint: str, user_id: int
    ) -> dict[str, Any]:
    ...
    async def run_test(self) -> dict[str, Any]:
        """Run the load test"""
        print(
            f"🚀 Starting load test: {self.concurrent_users} users for {self.duration}s"
        )
...
def main():
    ...
    for endpoint in results["endpoint_breakdown"]:
        print(f"  {endpoint}:")
        print(".2f")
        print(".3f")
    ...
    if args.output:
        with open(args.output, "w") as f:
            json.dump(
                {"summary": results, "detailed_results": tester.results}, f, indent=2
            )
