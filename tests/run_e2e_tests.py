...
async def run_tests(base_url: str = "http://localhost:8000",
    ...
    async with E2ETestFramework(base_url=base_url, ws_url=ws_url) as framework:
        try:
            ...
            print(f"\n❌ E2E Test Suite failed with error: {str(e)}")
            return 3

def main():
    parser = argparse.ArgumentParser(description='Run E2E tests for Simple378 Fraud Detection Platform')
    parser.add_argument('--base-url', default='http://localhost:8000',
                       help='Base URL for the API server (default: http://localhost:8000)')
    parser.add_argument('--ws-url', default='ws://localhost:8080',
