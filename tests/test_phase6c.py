...
mock_evidence_processor.process_files_batch.return_value = [
    MagicMock(
        metadata={}, extracted_text="test text", quality_score=0.8, key_entities=[]
    )
]

# Patch at the source
with (
    patch("backend.app.services.ai.ai_service.ai_service", mock_ai_service),
    patch(
        "backend.app.services.intelligence.evidence_service.evidence_processor",
...
def test_rag_add():
    ...
    r = client.post(
        "/api/v1/advanced-ai/rag/add", data=payload
...
def test_analyze_text():
    ...
    r = client.post(
        "/api/v1/advanced-ai/multimodal/text", data=payload
    )  # Use data instead of json for Form data
    assert r.status_code == 200
    body = r.json()
