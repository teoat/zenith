...
async def verify_full_stack():
    ...
    try:
        ...
        ui_plugin = await plugin_registry_service.get_plugin(
            "zenith/ui/fraud_metrics_widget", db
        )
        ...
        email_plugin = await plugin_registry_service.get_plugin(
            "zenith/integration/email_notifier", db
        )
        ...
        assign_plugin = await plugin_registry_service.get_plugin(
            "zenith/workflow/round_robin_assigner", db
        )
        ...
        logger.info(
            f"Assignments: {res1['assigned_agent']} -> {res2['assigned_agent']} -> {res3['assigned_agent']}"
        )

        # Verify rotation: A -> B -> A
        if (
            res1["assigned_agent"] == "Agent A"
            and res2["assigned_agent"] == "Agent B"
            and res3["assigned_agent"] == "Agent A"
        ):
