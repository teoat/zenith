...
{self._generate_js_response(method)}"""
    ...
    def _get_method(self, endpoint: str) -> str:
        ...
        if endpoint in ["/auth/register", "/auth/login", "/auth/refresh"]:
            return "POST"
        elif endpoint.startswith("/cases") and "{case_id}" not in endpoint:
            return "POST"
        elif endpoint.startswith("/evidence/upload"):
            return "POST"
        elif endpoint.startswith("/fraud"):
            return "POST"
        elif endpoint.startswith("/ai"):
            return "POST"
        elif endpoint.startswith("/reports/generate"):
            return "POST"
        elif "{case_id}" in endpoint or "{evidence_id}" in endpoint or "{job_id}" in endpoint:
...
<body>
    ...
    def save_examples(self):
        ...
        print(f"✅ API examples generated:")
        ...
        print(f"   - Interactive HTML documentation")
