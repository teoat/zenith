...
def extract_openapi_specs() -> dict[str, Any]:
    ...
    try:
        ...
        openapi_spec["info"]["x-generation-timestamp"] = str(
...
def create_markdown_documentation(
    ...
    for tag, endpoints in endpoints_by_tag.items():
        ...
        for endpoint in endpoints:
            ...
            # Examples
            example = next(
                (e for e in examples.get(tag, []) if e["path"] == path), None
            )
            if example:
                md.append("#### Examples")
                md.append("")
...
def save_documentation(
    ...
    print(
        f"   - Examples generated: {sum(len(ex) for ex in examples.values())} endpoints"
    )
...
def main():
    ...
    markdown_doc = create_markdown_documentation(
        openapi_spec, endpoints_by_tag, examples
    )
