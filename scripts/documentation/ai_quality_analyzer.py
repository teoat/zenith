...
class AIDocumentationAnalyzer:
    ...
    def _load_ai_rules(self) -> Dict[str, Any]:
        ...
        return {
            'content_generation_rules': {
                'minimum_length': 100,
                'required_sections': ['introduction', 'core_concepts', 'examples', 'troubleshooting'],
                'example_density': 0.1,
    ...
    def _calculate_readability(self, content: str) -> float:
        ...
        readability_factor = max(0, (25 - abs(avg_sentence_length - self.quality_patterns['readability_indicators']['sentence_length_avg'])) / 25)
        
        words = content.split()
        avg_word_length = sum(len(w) for w in words) / len(words)
        complexity_penalty = min(20, len([w for w in words if len(w) > 12]) / len(words)) * 10
    ...
    def _calculate_completeness(self, content: str) -> float:
        ...
        sections_found = sum(1 for condition in completeness_factors.values() if condition)
    ...
    def _calculate_consistency(self, content: str) -> float:
        ...
        if len(term_usage) > 5:
            variation_penalty = min(10, abs(max(term_usage.values()) - min(term_usage.values())) * 2)
        ...
        if '##' in content:
            ...
            header_levels = [len(h) for h in re.findall(r'^#+\s*(.+)$', content, re.MULTILINE)]
    ...
    def _calculate_grammar(self, content: str) -> float:
        ...
        grammar_issues = [
            ...
            (r'\b(i|you|your|we|our|us)\b', 3),  # First-person pronouns (context dependent)
        ]
    ...
    def _calculate_structure(self, content: str) -> float:
        ...
        # Table of contents
        if 'Table of Contents' in content and not re.search(r'^#{1,4}\s*Table of Contents$', content):
            structure_score -= 15
    ...
    def _calculate_accessibility(self, content: str) -> float:
        ...
        if re.search(r'\[.*\]\(\s*https?://\)', content):
            link_text_check = re.findall(r'\[.*\]\(\s*(https?://\w+\.\w+\.?\w+)\s*\)', content)
    ...
    def _adjust_content_score(self, content: str, base_metrics: QualityMetrics) -> float:
    ...
    def _analyze_with_ai(self, content: str, filename: str) -> Dict[str, Any]:
        ...
        if len(content) < 200:
            insights['issues'].append("Content too brief - minimum 200 characters recommended")
            insights['improvements'].append("Expand content with more detailed explanations")
        ...
        if 'Example' in filename and '```' not in content:
            ...
            insights['improvements'].append("Add formatted code examples with syntax highlighting")
        ...
        for issue in common_issues:
            if issue in content:
                insights['issues'].append(f"Issue marker '{issue}' found - resolve before publishing")
        
        return insights
    
    def generate_improvement_suggestions(self, metrics: QualityMetrics, file_path: Path) -> List[str]:
        ...
        if metrics.completeness_score < 80:
            suggestions.append("Add missing sections: Troubleshooting, Best Practices, Advanced Topics")
    ...
    def analyze_documentation_set(self, docs_dir: Path) -> Dict[str, Any]:
        ...
        aggregate_metrics = {
            ...
            'grade_distribution': {grade: sum(1 for m in all_metrics if m.overall_grade == grade) for grade in ['A', 'B', 'C', 'D', 'F']},
            'top_issues': [issue for m in all_metrics for issue in m.issues_found[:10]],
            'common_improvements': list(set([imp for m in all_metrics for imp in m.improvements_suggested[:5]]))
        }
    ...
    def _generate_set_recommendations(self, aggregate_metrics: Dict[str, Any]) -> List[str]:
        ...
        if avg_score < 70:
            recommendations.append("Comprehensive quality improvement needed - focus on accuracy and completeness")
            recommendations.append("Establish documentation review process")
            recommendations.append("Consider professional technical writing for complex topics")
        
        if avg_score < 85:
            recommendations.append("Enhance content with more examples and practical guidance")
            recommendations.append("Improve consistency across all documentation")
            recommendations.append("Add visual elements and diagrams for complex concepts")
        
        if aggregate_metrics['grade_distribution']['D'] > 0:
            recommendations.append("Address failing grade documents first - quality is critical")
            recommendations.append("Implement mandatory documentation standards")
            recommendations.append("Focus on fundamental improvements before advanced features")
    ...
    def save_analysis_results(self, results: Dict[str, Any], output_dir: Path) -> None:
        ...
        with open(summary_file, 'w') as f:
            f.write(f"# Documentation Quality Analysis\n\n")
            f.write(f"**Generated**: {results['metadata']['generated_at']}\n\n")
            f.write("## Summary\n\n")
            f.write(f"- Total Documents: {results['aggregate_metrics']['total_documents']}\n")
            f.write(f"- Average Quality Score: {results['aggregate_metrics']['average_quality_score']:.1f}\n")
            f.write(f"- Grade Distribution: {results['aggregate_metrics']['grade_distribution']}\n\n")
            f.write("## Top Issues\n\n")
