...
class DocumentationValidator:
    ...
    def validate_single_link(self, source_file: Path, link_text: str, link_url: str):
        ...
        if not target_path.exists():
            self.add_issue(f"Broken link in {source_file.name}: '{link_text}' -> {link_url}", "error")
    ...
    def validate_api_analysis(self, api_analysis_path: Path):
        ...
        try:
            ...
            if total_endpoints < 100:
                self.add_issue(f"Low endpoint coverage: {total_endpoints} endpoints found", "warning")
    ...
    def validate_markdown_file(self, file_path: Path):
        ...
        try:
            ...
            # Report issues
            for issue in issues:
                self.add_issue(f"Markdown quality issue in {file_path.name}: {issue}", "warning")
...
def generate_markdown_report(report: Dict[str, Any]) -> str:
    ...
    if report['issues']:
        ...
        for issue in report['issues']:
            ...
            md.append(f"{severity_emoji} **{issue['severity'].upper()}**: {issue['message']}")
            md.append("")
