...
class ComprehensiveDiagnosticOrchestrator:
    ...
    async def run_comprehensive_diagnosis(
        self, target_sectors: list[str] | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {
                    ...
                    "reason": "Docker not running or no containers",
                }
        except Exception as e:
    ...
    def _check_backend_configuration(self) -> dict[str, Any]:
        ...
        required_env_vars = ["DATABASE_URL", "SECRET_KEY", "ENVIRONMENT"]
    ...
    async def _check_frontend_runtime(self) -> dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession(
                timeout=aiohttp.ClientTimeout(total=5)
            ) as session, session.get(dev_server_url) as response:
                results["dev_server"] = {
    ...
    def _check_vector_store(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "last_updated": max(
                    [
                        doc.get("created_at", "1970-01-01")
                        for doc in ai_service.vector_store.values()
                    ]
                )
    ...
    def _collect_performance_metrics(self) -> dict[str, Any]:
        ...
        try:
            from app.services.infrastructure.performance_monitor import (
                performance_monitor,
            )
    ...
    def _check_unit_tests(self) -> dict[str, Any]:
        ...
        return {"backend": backend_tests, "frontend": frontend_tests}
    ...
    def _check_test_coverage(self) -> dict[str, Any]:
        ...
        coverage_dirs = ["backend/htmlcov", "frontend/coverage"]
    ...
    def _check_user_documentation(self) -> dict[str, Any]:
        ...
        for doc_file in doc_files:
            ...
            if path.exists():
                size = path.stat().st_size
                results[doc_file] = {"status": "exists", "size_bytes": size}
            else:
    ...
    def _check_system_dependencies(self) -> dict[str, Any]:
        ...
        system_tools = ["python3", "node", "npm", "docker", "git"]
    ...
    def _generate_summary(self):
        ...
        for sector_data in sectors.values():
            ...
            summary["critical_issues"] += len(
                [i for i in issues if "critical" in i.lower()]
            )
            summary["warning_issues"] += len(
                [i for i in issues if "warning" in i.lower() or "degraded" in i.lower()]
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive System Diagnostic Orchestrator"
    )
    parser.add_argument(
        "sectors",
        nargs="*",
