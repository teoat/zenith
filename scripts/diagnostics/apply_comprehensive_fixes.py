...
from app.services.enhanced_experimentation_platform import enhanced_experimentation_platform
...
async def main():
    ...
    experiment = await enhanced_experimentation_platform.create_experiment_from_template(
        'a_b_feature_rollout',
    ...
    success = await enhanced_experimentation_platform.start_experiment(experiment.experiment_id)
    ...
    for risk_data in sample_risks:
        ...
        for action_id, action in comprehensive_risk_mitigation.mitigation_actions.items():
            if action.risk_id == risk.risk_id:
                success = await comprehensive_risk_mitigation.execute_mitigation(action_id)
                if success:
    ...
    print(f"   📊 Automated fixes available: {analysis_results['automated_fixes_available']}")
    print(f"   📊 Estimated technical debt: {analysis_results['technical_debt_hours']:.1f} hours")
    ...
    if analysis_results['issues_by_type']:
        ...
        sorted_types = sorted(analysis_results['issues_by_type'].items(), key=lambda x: x[1], reverse=True)
    ...
    print(f"   📋 Estimated effort: {refactoring_plan['estimated_effort_days']:.1f} days")
    ...
    async def monitor_experiments():
        while True:
            status = await enhanced_experimentation_platform.get_experiment_status(experiment.experiment_id)
            if status.get('status') in ['completed', 'failed']:
                print(f"   📊 Experiment {experiment.experiment_id} finished: {status.get('status')}")
                break
    ...
    async def monitor_risks():
        while True:
            ...
            if dashboard['total_risks'] > 0:
                print(f"   🛡️ Risk status: {dashboard['total_risks']} total, {dashboard['mitigated_risks']} mitigated")
    ...
    summary = {
        ...
        'risk_mitigation': {
            ...
            'mitigations_applied': len([a for a in comprehensive_risk_mitigation.mitigation_actions.values() if a.status == 'monitoring']),
            'monitoring': 'active'
        },
        'code_quality': {
    ...
    print(f"   ✅ Total issues addressed: {analysis_results['issues_found'] + len(comprehensive_risk_mitigation.risks)}")
