...
NEEDS_AUTH = [
    'apm.py', 'graph.py', 'stats.py', 'cases.py', 
    'evidence.py', 'fraud.py', 'analytics.py',
    'notifications.py', 'logging.py', 'reporting.py'
]
...
def ensure_auth_imports(filepath: Path) -> bool:
    ...
    for i, line in enumerate(lines):
        ...
        if line.startswith('import ') or line.startswith('from '):
            ...
            if 'from core.database import' in line and ', User' not in line and not user_import_added:
                if line.endswith(')'):
                    # Multi-line import
    ...
    if not auth_import_added and last_import_line > 0:
        new_lines.insert(last_import_line + 1, 'from app.services.auth_service import auth_service')
    ...
    for i, line in enumerate(lines):
        ...
        if i > 100 and line.strip() == 'from app.services.auth_service import auth_service':
