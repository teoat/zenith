...
def get_router_info(router_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        info = {
            "name": router_path,
            "endpoints": [],
            "description": getattr(module, "__doc__", "").strip() if hasattr(module, "__doc__") and module.__doc__ else "",
            "tags": []
        }
        ...
        if hasattr(module, "router"):
            ...
            if hasattr(router, "routes") and router.routes:
                for route in router.routes:
                    ...
                    if hasattr(route, "endpoint") and route.endpoint and hasattr(route.endpoint, "__doc__"):
                        endpoint_info["docstring"] = route.endpoint.__doc__.strip() if route.endpoint.__doc__ else ""
...
"""
    ...
    print(f"Total endpoints documented: {sum(len(r.get('endpoints', [])) for r in all_routers)}")
