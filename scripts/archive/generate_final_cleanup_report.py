...
def generate_final_cleanup_report():
    ...
    findings = {
        ...
        "duplicate_files": {
            ...
            "docker_files": ["./Dockerfile", "./backend/Dockerfile"],
            "env_files": [
                ".env.example",
                ".env.production",
...
echo "⚠️  Manual review recommended for other cleanup items"
    ...
    with open(summary_path, "w") as f:
        ...
        f.write(
            f"- **Configuration Consolidation:** {effort['consolidate_configurations']}\n"
        )
    ...
    print(
        f"📊 Duplicates Found: {sum(len(files) for files in findings['duplicate_files'].values()) - len(findings['duplicate_files'])} groups"
    )
    print(f"📁 Potentially Unused: {len(findings['potentially_unused_files'])} files")
    print(
        f"⏱️ Estimated Effort: {findings['estimated_cleanup_effort']['total_estimated']}"
    )
