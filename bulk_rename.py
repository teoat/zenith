...
def bulk_replace(root_dir, replacements):
    ...
    extensions = ['*.py', '*.js', '*.ts', '*.tsx', '*.json', '*.md', '*.html', '*.yml', '*.yaml', '*.toml', '*.cfg', '*.ini', '*.txt']
    ...
    for ext in extensions:
        ...
        for filepath in files:
            ...
            if any(skip in filepath for skip in ['node_modules', '__pycache__', '.git', 'htmlcov', 'dist', 'build']):
                continue
...
def main():
    ...
    print("\nRunning validation...")
    os.system("cd /Users/Arief/Desktop/Zenith && python scripts/documentation/validate_docs.py 2>/dev/null | tail -10")
