...
def bulk_replace_remaining(root_dir):
    ...
    extensions = ['*.py', '*.js', '*.ts', '*.tsx', '*.json', '*.md', '*.html', '*.yml', '*.yaml', '*.toml', '*.cfg', '*.ini', '*.txt', '*.sh']
    ...
    for ext in extensions:
        ...
        try:
            ...
            for filepath in files:
                ...
                skip_patterns = [
                    'node_modules', '__pycache__', '.git', 'htmlcov', 'dist',
                    'coverage', 'monitoring_state.json', 'backend_startup.log',
                    'frontend_startup.log', 'test_results.json'
                ]
...
def main():
    ...
    print("Starting final comprehensive rename of all remaining Zenith → Zenith references...")
    ...
    os.chdir(root_dir)
    result = os.popen("python scripts/documentation/validate_docs.py 2>/dev/null | tail -10").read()
    print(result)

    # Check for remaining Zenith references
    print("\nChecking for any remaining Zenith references...")
    remaining = os.popen("find /Users/Arief/Desktop/Zenith -type f \\( -name '*.py' -o -name '*.js' -o -name '*.ts' -o -name '*.tsx' -o -name '*.json' -o -name '*.md' -o -name '*.html' -o -name '*.yml' -o -name '*.yaml' -o -name '*.toml' -o -name '*.cfg' -o -name '*.ini' -o -name '*.txt' -o -name '*.sh' \\) -exec grep -l 'Zenith' {} \\; 2>/dev/null | head -10").read()
    if remaining.strip():
