...
def cached_query(ttl: int = _cache_ttl):
    """Legacy decorator for caching query results - now uses distributed cache"""
    return distributed_cached_query(ttl=ttl)


async def clear_query_cache():
    """Legacy function to clear query cache - now uses distributed cache"""
    cache = get_cache()
...
class DatabaseOptimizer:
    ...
    def get_performance_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cache_size": asyncio.run(get_cache().backend.get_stats()).get("total_keys", 0),
            "timestamp": time.time(),
        }
