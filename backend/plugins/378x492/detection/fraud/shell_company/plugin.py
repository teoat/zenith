...
def detect_shell_companies(transactions: List[Dict[str, Any]]) -> List[ShellCompanyAlert]:
    """
...
class ShellCompanyPlugin(PluginInterface):
    ...
    def metadata(self) -> PluginMetadata:
        return PluginMetadata(
            name="shell_company",
            version="1.0.0",
            namespace="378x492/detection/fraud/shell_company",
            author="378x492 Team",
    ...
    async def initialize(self, context: PluginContext) -> bool:
        ...
        config_dict = context.config if context.config else {
            "min_transaction_volume": 1000.0,
            "pass_through_threshold": 0.05
        }
