...
async def require_admin(
    ...
    if current_user.role not in ["admin", "super_admin"]:
        ...
        raise HTTPException(
            status_code=403, 
            detail="Admin access required for backup operations"
        )
    return current_user
...
class BackupRequest(BaseModel):
    ...
    type: Literal["full", "incremental", "auto"] = Field("auto", description="Backup type")
    ...
    try:
        ...
        if request.type == "full":
            ...
            return BackupResponse(
                success=True,
                message=f"Full backup started by {admin.email}"
            )
            ...
            return BackupResponse(
                success=True,
                message="Incremental backup started in background"
            )
            ...
            return BackupResponse(
                success=True,
                message="Automatic backup started in background"
            )
...
async def get_backup_status(admin: User = Depends(require_admin)):
    ...
    try:
        logger.info(
            f"Admin {admin.id} accessing backup status",
...
async def list_backups(admin: User = Depends(require_admin)):
    ...
    try:
        logger.info(
            f"Admin {admin.id} listing backups",
            extra={"admin_id": admin.id}
        )
        backup_manager = await get_backup_manager()
...
async def delete_backup(
    ...
    try:
        ...
        backup_manager.backup_metadata['backups'] = [
            b for b in backup_manager.backup_metadata['backups']
            if b['id'] != backup_id
        ]
...
async def update_backup_config(
    ...
    try:
        ...
        valid_keys = {
            'retention_days', 'max_backup_size_gb', 'compression_level',
            'encryption_enabled', 'remote_backup_enabled', 'remote_backup_url',
            'backup_schedule', 'full_backup_interval_days', 'incremental_backup_enabled'
        }
    ...
    try:
        ...
        if not last_full:
            ...
            if days_since_full >= backup_manager.config['full_backup_interval_days']:
                ...
                result = await backup_manager.create_incremental_backup(reason)
                if result:
                    logger.info(f"Automatic incremental backup completed: {result['id']}")
                else:
...
async def backup_health_check():
    ...
    try:
        ...
        health_status = {
            ...
            "components": {
                "backup_directory": {
                    "status": "healthy" if backup_manager.backup_base_dir.exists() else "unhealthy",
                    "path": str(backup_manager.backup_base_dir)
                },
                "database": {
                    "status": "healthy" if backup_manager.config['database_path'] else "unhealthy",
                    "path": backup_manager.config['database_path']
                },
                "metadata": {
                    "status": "healthy",
                    "backups_count": len(backup_manager.backup_metadata.get('backups', []))
                }
            },
