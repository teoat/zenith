...
from app.services.intelligence.timeline_reconstruction import (
...
class InvestigationGenerationRequest(BaseModel):
    ...
    case_type: str = (
        "fraud_detection"  # aml_investigation, fraud_detection, network_analysis, compliance_review
    )
        ...
        await audit_service.log_access(
            ...
            details={
                ...
                "actions_generated": len(
                    investigation_result.get("generated_actions", [])
                ),
                "ai_enhanced": request.ai_enhanced,
                "processing_time_ms": processing_time,
            },
...
async def generate_timeline(
    ...
    try:
        ...
        validation_result = await timeline_engine.validate_timeline_integrity(
            timeline_result
        )

        # Optimize timeline
        optimized_timeline = (
            await timeline_engine.optimize_timeline(timeline_result)
            if timeline_result
            else None
        )
        ...
        return TimelineGenerationResponse(
            ...
            total_duration=(
                optimized_timeline.get("total_duration") if optimized_timeline else None
            ),
            integrity_score=(
...
async def update_monitoring(
    ...
    """
    try:
        background_tasks.add_task(optimize_investigation_performance, db=db)
