...
async def get_node_details(
    node_id: str,
    include_neighborhood: bool = Query(True, description="Include neighborhood analysis"),
...
async def get_suspicious_patterns(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get suspicious patterns: {str(e)}")
...
async def get_high_risk_entities(
    ...
    try:
        ...
        high_risk_entities = graph_analyzer.get_high_risk_entities(min_risk_score, limit)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get high-risk entities: {str(e)}")
...
async def export_graph(
    ...
    try:
        ...
        export_data = graph_analyzer.export_graph(format, center_node, radius, include_neighbors)
...
async def search_graph(
    ...
    try:
        ...
        for node_id, node_data in graph_analyzer.graph.nodes(data=True):
            ...
            if query_lower in node_label.lower() or query_lower in str(node_properties.get('name', '').lower()):
                matching_nodes.append({
                    "id": node_id,
                    "label": node_label,
                    "type": node_type_val,
                    "risk_score": graph_analyzer._calculate_node_risk_score(node_id)
                })
