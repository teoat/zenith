...
from core.database import get_db, Case, Transaction, Entity, User
...
@router.get("")
async def get_cases(page: int = 1, per_page: int = 20, status: Optional[str] = None,
...
router.get("/")
async def get_cases_root(page: int = 1, per_page: int = 20, status: Optional[str] = None,
                         priority: Optional[str] = None, search: Optional[str] = None):
...
async def update_case(case_id: str, update_data: dict):
    ...
    try:
        ...
        case = db_service.update_case(case_id, mapped_data, mapped_data.get('updated_by', 'test_user'))
        if not case:
            raise HTTPException(status_code=404, detail="Case not found")
