...
from core.database import get_db, Relationship, GraphSnapshot, User
...
for _svc in ('relationship_graph', 'metadata_correlation_engine', 'graph_snapshot_service'):
...
async def get_graph_snapshots(
    ...
    try:
        snapshots = db.query(GraphSnapshot).filter(
            GraphSnapshot.case_id == case_id
        ).order_by(GraphSnapshot.created_at.desc()).all()
...
async def build_relationship_graph(
    ...
    entity_types: Optional[List[str]] = Query(None, description="Entity types to include")
):
...
async def get_central_entities(top_n: int = Query(10, description="Number of top entities to return")):
...
async def get_suspicious_patterns(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to find suspicious patterns: {str(e)}")
