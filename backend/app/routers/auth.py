...
async def login(login_data: LoginRequest, request: Request):
    ...
    try:
        ...
        if not user:
            ...
            logger.warning(f"Failed login attempt for username: {login_data.username} from IP: {request.client.host if request.client else 'unknown'}")
            raise HTTPException(
        ...
        logger.info(f"Successful login for user: {user.username} (ID: {user.id}) from IP: {request.client.host if request.client else 'unknown'}")
        ...
        try:
            ...
            user_journey_tracker.track_event(
                user_id=user.id,
                event_type="login",
                metadata={"role": user.role.value if user.role else None, "mfa": user.mfa_enabled}
            )
...
async def register(user_data: UserCreateRequest):
    ...
    try:
        ...
        if auth_service.get_user_by_username(user_data.username) or auth_service.get_user_by_email(user_data.email):
            raise HTTPException(
