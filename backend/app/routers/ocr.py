...
async def process_receipt(file: UploadFile = File(...)):
    ...
    try:
        contents = await file.read()
        return await receipt_processor.process_receipt(
            contents, file.filename or "receipt"
        )
    except Exception as e:
        raise HTTPException(
...
async def process_invoice(file: UploadFile = File(...)):
    ...
    try:
        contents = await file.read()
        return await invoice_processor.process_invoice(
            contents, file.filename or "invoice"
        )
    except Exception as e:
        raise HTTPException(
