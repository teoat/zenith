...
async def get_threat_map_locations(
    ...
    try:
        ...
        blocked_transactions = db.query(Transaction).filter(
            Transaction.status.in_(['denied', 'blocked'])
        ).filter(
            Transaction.city.isnot(None),
            Transaction.country.isnot(None)
        ).limit(50).all()
...
def get_dashboard_metrics(
    ...
    try:
        ...
        blocked_amount_result = db.query(func.sum(Transaction.amount))\
...
def _get_sparkline_data(db: Session) -> Dict[str, List[int]]:
    ...
    def get_daily_counts(model, date_col):
        ...
        records = db.query(getattr(model, date_col))\
            .filter(getattr(model, date_col) >= start_date).all()
    ...
    critical_records = db.query(Case.created_at)\
        .filter(Case.created_at >= start_date, Case.priority == 'critical').all()
    ...
    return {
        ...
        "openCases": [max(0, x - 1) for x in total_cases_trend], # rudimentary mock for "open" flux
