...
async def get_compliance_dashboard(
    ...
    overdue_access_reviews = (
        db.query(AccessReview).filter(AccessReview.review_status == "overdue").count()
    )
    ...
    last_100_logs = (
        db.query(ComplianceAuditLog.risk_score)
...
async def get_monitoring_dashboard(
    ...
    active_alerts_db = (
        db.query(FraudAlert)
        .filter(FraudAlert.is_acknowledged == False)
    ...
    return {
        "system_health": {
            ...
            "error_rate": 0.01,
            "active_users": db.query(AccessReview)
            .distinct(AccessReview.user_id)
            .count()
            or 42,  # Mock estimation
...
async def log_audit_event(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Failed to log audit event: {e!s}")
