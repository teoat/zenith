...
from app.services.apm_service import get_apm_summary, record_metric, start_span, finish_span, create_alert
...
@router.get("/summary")
async def get_apm_summary_endpoint(current_user: User = Depends(auth_service.get_current_user)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get APM summary: {str(e)}")
...
async def finish_span_endpoint(
    ...
    error_message: Optional[str] = Body(None, description="Error message if status is error"),
    db: Session = Depends(get_db)
):
    """
...
async def export_apm_data(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to export APM data: {str(e)}")
