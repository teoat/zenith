...
class MultiModalAnalyzer:
    ...
    def _check_image_analysis_available(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Image analysis not available - install Pillow and opencv-python"
            )
            return False
    ...
    def _check_document_analysis_available(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Document analysis not available - install pymupdf, python-docx, and pandas"
            )
            return False
    ...
    def _check_forensic_availability(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Forensic analysis not available - install required dependencies"
            )
            return False
    ...
    def _init_modules(self):
        ...
        logger.info(
            f"Analysis modules - OCR: {self.ocr_available}, "
        ...
        try:
            ...
            if file_type.startswith("image/"):
                ...
                analysis.errors.append(
                    f"Unsupported file type for analysis: {file_type}"
                )

            # Perform forensic analysis if available
            if self.forensic_available and options.get("enable_forensics", True):
                analysis.forensic_result = self._perform_forensic_analysis(
                    file_path, analysis
                )
            ...
            logger.info(
                f"Multi-modal analysis completed for {evidence_id} in {analysis.processing_time:.2f}s"
            )
    ...
    def _analyze_image_file(
        self, file_path: str, analysis: MultiModalAnalysis, options: Dict[str, Any]
    ):
        ...
        try:
            ...
            if self.ocr_available and options.get("enable_ocr", True):
                ...
                if analysis.extracted_text:
                    analysis.key_entities = self._extract_entities_from_text(
                        analysis.extracted_text
                    )
                    analysis.sentiment_score = self._analyze_sentiment(
                        analysis.extracted_text
                    )
                    analysis.language_detected = self._detect_language(
                        analysis.extracted_text
                    )
    ...
    def _analyze_word_file(
        ...
        try:
            ...
            analysis.document_structure = {
                ...
                "has_comments": (
                    len(doc.comments) > 0 if hasattr(doc, "comments") else False
                ),
    ...
    def _extract_tables_from_text(self, text: str) -> List[TableData]:
        ...
        for line in lines:
            ...
            if len(stripped_line.split()) > 2 and len(stripped_line) > 20:
                ...
                if (
                    len(current_table_lines) > 1
                ):  # Require at least a header and one row
    ...
    def _parse_text_table(
        ...
        first_line_cells = [
            cell.strip() for cell in table_lines[0].split(delimiter) if cell.strip()
        ]
        if all(
            word[0].isupper() or not word.isalpha() for word in first_line_cells if word
        ...
        if (
            headers and not rows
        ):  # If only headers were detected, but no rows matched the delimiter logic, re-evaluate
    ...
    def _perform_forensic_analysis(
        self, file_path: str, analysis: MultiModalAnalysis
    ) -> ForensicResult:
    ...
    def _analyze_general_forensics(
        self, file_path: str, analysis: MultiModalAnalysis
    ) -> ForensicResult:
        ...
        try:
            ...
            if file_mime != declared_mime:
                forensic_indicators.append(
                    f"File type mismatch: declared {declared_mime}, actual {file_mime}"
                )
                manipulation_score += 20
                authenticity_score -= 15
    ...
    def _analyze_noise_pattern(self, image: "Image") -> Dict[str, Any]:
        ...
        try:
            ...
            inconsistent = any(
                abs(noise - avg_noise) / avg_noise > 0.5 for noise in noise_levels
            )
    ...
    def _detect_clone_regions(self, image: "Image") -> Dict[str, Any]:
        ...
        try:
            ...
            return {"clones_detected": clones_detected, "template_size": template_size}
    ...
    def _has_suspicious_pdf_metadata(self, metadata: Dict[str, Any]) -> bool:
        ...
        if any(
            suspicious in software.lower()
            for suspicious in ["unknown", "fake", "anonymous"]
        ):
