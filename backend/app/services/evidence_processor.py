...
class MultiModalProcessor:
    ...
    def process_file(self, file_path: str, file_id: Optional[str] = None) -> ExtractedEvidence:
        """
        ...
        if extension in self.SUPPORTED_IMAGE_FORMATS:
            evidence = self._process_image(file_bytes, file_path.name, file_id or file_hash)
        elif extension == '.pdf':
            evidence = self._process_pdf(file_bytes, file_path.name, file_id or file_hash)
        elif extension == '.txt':
            evidence = self._process_text(file_bytes, file_path.name, file_id or file_hash)
        else:
    ...
    def _process_image(self, image_bytes: bytes, filename: str, file_id: str) -> ExtractedEvidence:
    ...
    def _process_pdf(self, pdf_bytes: bytes, filename: str, file_id: str) -> ExtractedEvidence:
    ...
    def _process_text(self, text_bytes: bytes, filename: str, file_id: str) -> ExtractedEvidence:
    ...
    def _analyze_image_forensics(self, image: Image.Image, metadata: Dict) -> Dict[str, Any]:
        """
        ...
        forensics['risk_level'] = (
            'high' if len(forensics['suspicious_factors']) >= 2
            else 'medium' if forensics['suspicious_factors']
            else 'low'
        )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'by_type': {
                'images': len([e for e in self.processed_files if e.file_type == 'image']),
                'pdfs': len([e for e in self.processed_files if e.file_type == 'pdf']),
                'text': len([e for e in self.processed_files if e.file_type == 'text']),
            },
            'total_extracted_chars': sum(len(e.extracted_text) for e in self.processed_files),
            'avg_ocr_confidence': sum(e.ocr_confidence for e in self.processed_files) / len(self.processed_files) if self.processed_files else 0,
            'suspicious_images': len([e for e in self.processed_files 
                                     if e.image_analysis and e.image_analysis.get('forensics', {}).get('risk_level') == 'high'])
        }
