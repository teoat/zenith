...
class FraudRule:
    ...
    def __init__(self, rule_id: str, name: str, description: str, rule_type: RuleType, 
                 risk_level: RiskLevel, enabled: bool = True):
...
class VelocityRule(FraudRule):
    ...
    def __init__(self, rule_id: str, max_transactions: int, time_window_minutes: int, 
...
class AmountThresholdRule(FraudRule):
    ...
    def __init__(self, rule_id: str, threshold_amount: float, currency: str = "USD",
...
class GeographicAnomalyRule(FraudRule):
    ...
    def __init__(self, rule_id: str, max_distance_km: float = 500, 
                 min_time_hours: float = 1, risk_level: RiskLevel = RiskLevel.CRITICAL):
        super().__init__(
            rule_id=rule_id,
    ...
    def evaluate(self, transaction: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not last_transaction:
            return {"triggered": False, "risk_score": 0, "reason": "No previous transaction", "details": {}}
...
class RuleEngine:
    ...
    def _generate_recommendations(self, triggered_rules: List[Dict], 
...
def create_default_rules() -> RuleEngine:
    ...
    # Velocity rules
    engine.add_rule(VelocityRule("VEL001", max_transactions=5, time_window_minutes=5, risk_level=RiskLevel.HIGH))
    engine.add_rule(VelocityRule("VEL002", max_transactions=10, time_window_minutes=60, risk_level=RiskLevel.MEDIUM))
    ...
    # Geographic rules
    engine.add_rule(GeographicAnomalyRule("GEO001", max_distance_km=500, min_time_hours=1, risk_level=RiskLevel.CRITICAL))
