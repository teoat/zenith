...
def _find_round_trip_paths(
    graph: Dict[str, List[Dict]], start_entity: str, max_path_length: int
) -> List[List[Dict]]:
    ...
    def dfs(current_entity: str, path: List[Dict], visited: Set[str]):
        ...
        if (
            len(path) >= 2
            and current_entity == start_entity
...
def _create_round_trip_alert(
    trip_path: List[Dict], all_transactions: List[Dict[str, Any]]
) -> RoundTripAlert:
