    ...
    for tx in transactions:
        try:
            ...
            if prediction["score"] >= score_threshold:
                ...
                logger.debug(
                    f"AI alert generated for transaction {tx.get('id')}: score={prediction['score']}"
                )

        except Exception as e:
            logger.error(
                f"Error analyzing transaction {tx.get('id')} with AI: {str(e)}"
            )
            continue
