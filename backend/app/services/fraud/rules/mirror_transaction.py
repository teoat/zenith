...
def detect_mirror_transactions(transactions: List[Dict[str, Any]], time_window_minutes: int = 60, amount_tolerance: float = 0.01) -> List[MirrorTransactionAlert]:
    """
    ...
    for i in range(len(sorted_tx)):
        ...
        for j in range(i + 1, len(sorted_tx)):
            ...
            if type1 and type2 and type1 != type2:
                ...
                alerts.append(MirrorTransactionAlert(
                    transaction_ids=[tx1.get('id'), tx2.get('id')],
                    amount=amt1,
                    time_diff_seconds=time_diff,
                    confidence=0.9 - (amount_diff_percent * 10) # Higher confidence for exact matches
                ))
