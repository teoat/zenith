...
def detect_structuring(transactions: List[Dict[str, Any]], 
                      reporting_limit: float = 10000.0,
                      structuring_threshold: float = 0.9,
                      time_window_days: int = 30) -> List[StructuringAlert]:
    """
    ...
    for tx in transactions:
        entity = tx.get('customer_id') or tx.get('account_id') or tx.get('customer_name')
        if not entity:
            continue
    ...
    for entity, txs in entity_transactions.items():
        ...
        daily_alerts = _detect_daily_structuring(entity, txs, reporting_limit, structuring_threshold)
        alerts.extend(daily_alerts)
        
        # Pattern 2: Monthly limit avoidance
        monthly_alerts = _detect_monthly_structuring(entity, txs, reporting_limit, structuring_threshold)
        alerts.extend(monthly_alerts)
        
        # Pattern 3: Frequent small transactions (smurfing)
        smurfing_alerts = _detect_smurfing_pattern(entity, txs, reporting_limit, structuring_threshold)
        alerts.extend(smurfing_alerts)
    
    return alerts

def _detect_daily_structuring(entity: str, transactions: List[Dict], 
                            reporting_limit: float, threshold: float) -> List[StructuringAlert]:
    ...
    for date, day_txs in daily_groups.items():
        ...
        if total_amount >= reporting_limit:
            ...
            under_limit_count = sum(1 for tx in day_txs if tx['amount'] < reporting_limit * threshold)
            
            if under_limit_count >= len(day_txs) * 0.8:  # 80% of transactions under threshold
...
def _detect_monthly_structuring(entity: str, transactions: List[Dict],
                              reporting_limit: float, threshold: float) -> List[StructuringAlert]:
    ...
    for (year, month), month_txs in monthly_groups.items():
        ...
        if total_amount >= reporting_limit * 2:
            ...
            similar_count = sum(1 for amount in amounts if 
                             amount < reporting_limit * threshold and
                             abs(amount - avg_amount) / avg_amount < 0.2)  # Within 20% of average
...
def _detect_smurfing_pattern(entity: str, transactions: List[Dict],
                           reporting_limit: float, threshold: float) -> List[StructuringAlert]:
    ...
    if total_amount >= reporting_limit:
        ...
        if len(small_txs) >= 3:
            ...
            for i in range(1, len(small_txs)):
                interval = (small_txs[i]['date'] - small_txs[i-1]['date']).total_seconds() / 3600  # hours
                intervals.append(interval)
            
            # Check if intervals are somewhat regular
            if intervals:
                avg_interval = sum(intervals) / len(intervals)
                regular_count = sum(1 for interval in intervals if abs(interval - avg_interval) / avg_interval < 0.5)
