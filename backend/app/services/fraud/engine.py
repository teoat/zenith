...
from .rules.ai_detection import detect_ai_fraud, AIDetectionAlert
...
class FraudRule(ABC):
    ...
    @abstractmethod
    def execute(self, transactions: List[Dict[str, Any]], context: Dict[str, Any] = None) -> List[FraudAlert]:
...
class RuleEngine:
    ...
    def execute_rules(self, transactions: List[Dict[str, Any]], 
                      context: Dict[str, Any] = None) -> List[FraudAlert]:
        ...
        logger.info(f"Rule engine execution completed in {(execution_end - execution_start).total_seconds():.3f}s, generated {len(all_alerts)} alerts")
...
class MirrorTransactionRule(FraudRule):
    ...
    def execute(self, transactions: List[Dict[str, Any]], context: Dict[str, Any] = None) -> List[FraudAlert]:
        ...
        for raw_alert in raw_alerts:
            alert = FraudAlert(
                ...
                recommendations=["Review transaction sequence", "Check for money laundering patterns"]
            )
            alerts.append(alert)
    ...
    def get_config_schema(self) -> Dict[str, Any]:
        return {
            "time_window_minutes": {"type": "integer", "default": 60, "description": "Time window for mirror detection"},
            "amount_tolerance": {"type": "float", "default": 0.01, "description": "Amount tolerance for matching"}
        }
...
class ShellCompanyRule(FraudRule):
    ...
    def execute(self, transactions: List[Dict[str, Any]], context: Dict[str, Any] = None) -> List[FraudAlert]:
        ...
        for raw_alert in raw_alerts:
            alert = FraudAlert(
                ...
                recommendations=["Conduct enhanced due diligence", "Review business registration documents"]
            )
            alerts.append(alert)
    ...
    def get_config_schema(self) -> Dict[str, Any]:
        return {
            "min_transaction_volume": {"type": "float", "default": 1000, "description": "Minimum volume to analyze"},
            "pass_through_threshold": {"type": "float", "default": 0.05, "description": "Pass-through detection threshold"}
        }
...
class StructuringRule(FraudRule):
    ...
    def execute(self, transactions: List[Dict[str, Any]], context: Dict[str, Any] = None) -> List[FraudAlert]:
        ...
        for raw_alert in raw_alerts:
            alert = FraudAlert(
                ...
                recommendations=["File SAR if applicable", "Review transaction history", "Consider account monitoring"]
            )
            alerts.append(alert)
    ...
    def get_config_schema(self) -> Dict[str, Any]:
        return {
            "reporting_limit": {"type": "float", "default": 10000, "description": "Reporting threshold amount"},
            "structuring_threshold": {"type": "float", "default": 0.9, "description": "Threshold for structuring detection"}
        }
...
class RoundTripRule(FraudRule):
    ...
    def execute(self, transactions: List[Dict[str, Any]], context: Dict[str, Any] = None) -> List[FraudAlert]:
        ...
        for raw_alert in raw_alerts:
            alert = FraudAlert(
                ...
                recommendations=["Immediate investigation required", "Check for money laundering", "Review all parties involved"]
            )
            alerts.append(alert)
    ...
    def get_config_schema(self) -> Dict[str, Any]:
        return {
            "max_path_length": {"type": "integer", "default": 5, "description": "Maximum path length to analyze"},
            "time_window_hours": {"type": "float", "default": 24, "description": "Time window for round-trip detection"}
        }
...
class AIDetectionRule(FraudRule):
    ...
    def get_config_schema(self) -> Dict[str, Any]:
        return {
            "score_threshold": {"type": "float", "default": 60.0, "description": "Minimum fraud score to generate alert"},
            "model_path": {"type": "string", "default": "models/isolation_forest.pkl", "description": "Path to trained AI model"},
            "contamination": {"type": "float", "default": 0.1, "description": "Expected proportion of anomalies in training data"}
        }