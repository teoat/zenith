...
class BehaviorBaselineService:
    ...
    async def track_comingling_ratio(
        self, account_id: str, transactions: List[Dict[str, Any]], initial_personal_buffer: float = 1000.0
    ) -> Dict[str, Any]:
        """
        ...
        sorted_txs = sorted(transactions, key=lambda x: x.get("timestamp") or x.get("date") or datetime.now())
