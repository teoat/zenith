...
class AMLVelocityService:
    ...
    async def link_ubo(
        ...
        try:
            ...
            jurisdictions = list(set(c.get('jurisdiction', 'Unknown') for c in ownership_chain))
            
            # Check for high-risk jurisdictions
            high_risk_jurisdictions = ['Cayman Islands', 'British Virgin Islands', 'Panama', 'Seychelles', 'Bahamas']
    ...
    async def _trace_ownership(
        ...
        """Trace ownership structure (placeholder for graph DB query)."""
        # In production, this would query a corporate registry or graph database
        # For now, return empty list - actual data comes from external sources
        try:
