        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    evidence_id, file_path, content, content, key_entities,
                    metadata, file_type, quality_score, sentiment_score
                ))
    ...
    def search_evidence(self, query: str, limit: int = 20, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in cursor:
                    evidence_id, file_path, extracted_text, key_entities, metadata, file_type, quality_score, sentiment_score, indexed_at, rank = row
