...
class TemporalBurstDetector:
            ...
            try:
                ...
                if isinstance(date_value, str):
                    ...
                    for fmt in ["%Y-%m-%dT%H:%M:%S", "%Y-%m-%dT%H:%M:%SZ", "%Y-%m-%d %H:%M:%S", "%Y-%m-%d"]:
                        try:
                            date_value = datetime.strptime(date_value.split('.')[0].replace('Z', ''), fmt.replace('Z', ''))
                            break
                        except ValueError:
                            continue
                    else:
    ...
    def _detect_structuring(self, transactions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for customer_id, customer_txns in by_customer.items():
            ...
            suspicious = [
                t for t in customer_txns
                if self.structuring_lower_bound <= t['amount'] < self.structuring_threshold
            ]
            ...
            if len(suspicious) >= 3:  # At least 3 suspicious transactions
                ...
                avg_distance = mean(
                    (self.structuring_threshold - t['amount']) / self.structuring_threshold
                    for t in suspicious
                )
    ...
    def _calculate_risk_score(self, alerts: List[Dict], total_transactions: int) -> float:
        ...
        pattern_types = set(a['type'] for a in alerts)
