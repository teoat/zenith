...
class MonitoringService:
    ...
    def record_metric(self, name: str, value: float, tags: Dict[str, str] = None, metric_type: str = 'gauge'):
        if tags is None:
            tags = {}
        m = Metric(name=name, value=value, timestamp=datetime.now(), tags=tags, metric_type=metric_type)
        with self.metrics_lock:
            self.metrics.append(m)

    def record_request(self, response_time: float, status_code: int = 200, method: str = 'GET', endpoint: str = '/'):
        self.request_count += 1
        with self.metrics_lock:
            self.response_times.append(response_time)

    def record_error(self, error_type: str, message: str, metadata: Dict[str, Any] = None):
        if metadata is None:
            metadata = {}
        self.error_count += 1
        ev = ErrorEvent(timestamp=datetime.now(), error_type=error_type, message=message, stack_trace='', user_id=metadata.get('user_id'), session_id=metadata.get('session_id'), metadata=metadata)
    ...
    def get_health_metrics(self) -> Dict[str, Any]:
        ...
        score = 100
        if latest:
            if latest.cpu_percent > 90: score -= 20
            if latest.memory_percent > 90: score -= 20
            if latest.error_count > 50: score -= 20
...
def create_monitoring_middleware():
    ...
    async def middleware(request: Request, call_next):
        ...
        try:
            ...
            monitoring_service.record_error('unhandled_exception', str(e), {'path': str(request.url), 'method': request.method})
            raise
