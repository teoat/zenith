...
class AuditService:
    ...
    def log_request(self, user_id: Optional[str], session_id: Optional[str],
                   method: str, endpoint: str, status_code: int,
                   ip_address: Optional[str] = None, user_agent: Optional[str] = None,
                   response_size: Optional[int] = None, processing_time: Optional[float] = None,
    ...
    def _is_suspicious_endpoint(self, endpoint: str) -> bool:
        ...
        sensitive_patterns = [
            '/admin', '/config', '/system', '/backup', '/audit'
        ]
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.execute("""
                    ...
                    WHERE timestamp < datetime('now', '-{} days')
                """.format(self.retention_days * 2))  # Keep compliance events longer
    ...
    def verify_audit_integrity(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute("SELECT id, checksum, timestamp, user_id, action, resource_type, resource_id, method, endpoint, ip_address, user_agent, status_code, response_size, processing_time, details FROM audit_log")
