...
class RelationshipGraph:
    ...
    def build_graph_from_transactions(self, transactions: List[Dict[str, Any]]) -> nx.Graph:
    ...
    def _extract_entities(self, transactions: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        """Extract unique entities from transactions"""
        entities = {}

        for tx in transactions:
            # Customer/Account entity
            customer_id = tx.get('customer_id') or tx.get('account_id') or f"account_{tx.get('id', 'unknown')}"
    ...
    def _build_transaction_edges(self, transactions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for (entity1, entity2), tx_list in entity_pairs.items():
            ...
            edges.append({
                ...
                'attributes': {
                    ...
                    'first_transaction': min(tx.get('date') for tx in tx_list if tx.get('date')),
                    'last_transaction': max(tx.get('date') for tx in tx_list if tx.get('date'))
                }
            })
    ...
    def detect_shell_networks(
        ...
        for community_id, nodes in enumerate(communities):
            ...
            external_edges = total_edges - internal_edges if total_edges > internal_edges else 0
            ...
            if internal_ratio >= min_internal_ratio:
                risk_factors.append(f"High internal transaction ratio: {internal_ratio:.1%}")
                risk_score += 25
            
            # Presence of cycles indicates circular transactions
            if cycle_count > 0:
                risk_factors.append(f"Circular transaction patterns detected: {cycle_count} cycles")
    ...
    def find_central_entities(self, top_n: int = 10) -> List[Dict[str, Any]]:
        ...
        for node in self.graph.nodes():
            ...
            entities.append({
                ...
                'transaction_count': self.graph.nodes[node].get('total_transactions', 0)
            })
        ...
        if format == 'json':
            ...
            for node_id, node_data in self.graph.nodes(data=True):
                nodes.append({
                    ...
                    'type': node_data.get('type', 'unknown'),
                    'size': max(10, min(50, node_data.get('total_transactions', 1) * 2)),
                    'total_amount': node_data.get('total_amount', 0),
                    'transaction_count': node_data.get('total_transactions', 0),
                    **node_data
                })
    ...
    def get_graph_stats(self) -> Dict[str, Any]:
        ...
        edge_weights = [data.get('weight', 1) for _, _, data in self.graph.edges(data=True)]
        if edge_weights:
