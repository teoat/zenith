...
class PIIScrubber:
    ...
    PATTERNS = {
        ...
        'phone': (r'\b(?:\+?1[-.\s]?)?\(?[0-9]{3}\)?[-.\s]?[0-9]{3}[-.\s]?[0-9]{4}\b', '[PHONE_REDACTED]'),
        ...
        'api_key': (r'\b(?:api[_-]?key|apikey|token)[=:\s]+[A-Za-z0-9_\-]{16,}\b', '[API_KEY_REDACTED]'),
    ...
    def scrub_pii(text: str) -> str:
        ...
        result = text
        for pii_type, (pattern, replacement) in PIIScrubber.PATTERNS.items():
    ...
    def scrub_dict(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {}
        sensitive_keys = {'password', 'token', 'secret', 'key', 'auth', 'credential', 'ssn', 'credit_card'}
        ...
        for key, value in data.items():
            ...
            if any(sk in key_lower for sk in sensitive_keys):
                ...
                result[key] = [
                    PIIScrubber.scrub_dict(v) if isinstance(v, dict)
                    else PIIScrubber.scrub_pii(v) if isinstance(v, str)
                    else v
                    for v in value
                ]
            else:
