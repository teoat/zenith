...
class CostOptimizationService:
    ...
    def _calculate_progress_metrics(self, optimizations: List[CostOptimization]) -> Dict[str, Any]:
        ...
        actual_savings = sum(opt.actual_savings for opt in optimizations
    ...
    def _identify_next_milestones(self, optimizations: List[CostOptimization]) -> List[Dict[str, Any]]:
        ...
        sorted_opts = sorted(optimizations, key=lambda x: (x.timeline_months, x.estimated_savings), reverse=True)
    ...
    def get_cost_optimization_dashboard(self) -> Dict[str, Any]:
        ...
        actual_annual_savings = sum((opt.actual_savings or 0) * 12 for opt in self.optimizations.values()
    ...
    def _get_timeline_progress(self) -> Dict[str, Any]:
        ...
        current_savings = sum(opt.estimated_savings for opt in self.optimizations.values()
        ...
        return {
            ...
            'long_term_progress': min(current_savings / (quick_wins_target + medium_term_target + long_term_target), 1.0),
    ...
    def _generate_cost_recommendations(self) -> List[str]:
        ...
        completion_rate = len([opt for opt in self.optimizations.values() if opt.status == 'completed']) / len(self.optimizations) if self.optimizations else 0
    ...
    def _create_optimization_roadmap(self, category_analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        sorted_opportunities = sorted(
            all_opportunities,
            key=lambda x: (x['roi_percentage'], {'critical': 4, 'high': 3, 'medium': 2, 'low': 1}[x['priority']]),
            reverse=True
        )
        ...
        roadmap = {
            'phase_1_quick_wins': {
                'duration_months': 3,
                'opportunities': [opp for opp in sorted_opportunities if opp['payback_period_months'] < 6 and opp['complexity'] == 'Low'][:5],
                'total_savings': sum(opp['potential_savings'] for opp in sorted_opportunities if opp['payback_period_months'] < 6 and opp['complexity'] == 'Low'),
                'total_investment': sum(opp['implementation_cost'] for opp in sorted_opportunities if opp['payback_period_months'] < 6 and opp['complexity'] == 'Low')
            },
            'phase_2_high_impact': {
                'duration_months': 6,
    ...
    async def get_cost_dashboard(self) -> Dict[str, Any]:
        ...
        for opp in self.optimization_opportunities.values():
            ...
            if cat not in by_category:
                by_category[cat] = {'count': 0, 'potential_savings': 0, 'achieved_savings': 0}
        ...
        top_opportunities = sorted([
            ...
            for opp in self.optimization_opportunities.values()
        ], key=lambda x: x['roi_percentage'], reverse=True)[:10]
