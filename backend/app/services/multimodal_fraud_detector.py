...
class MultiModalFraudDetector:
    ...
    async def analyze_entity(self, entity_id: str, context_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        detected_patterns = await self._detect_fraud_patterns(entity_id, analysis_results)

        # Generate recommendations
        recommendations = self._generate_recommendations(combined_risk_score, detected_patterns)
    ...
    async def _analyze_behavioral_biometrics(self, entity_id: str,
    ...
    async def _build_behavioral_profile(self, entity_id: str,
                                      behavioral_data: Dict[str, Any]) -> BehavioralProfile:
    ...
    def _detect_behavioral_anomalies(self, profile: BehavioralProfile,
    ...
    async def _analyze_social_network(self, entity_id: str,
                                    context_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for tx in transactions:
            if tx.get('from_entity') == entity_id:
                ...
                if connected_entity:
                    ...
                    self._add_social_connection(entity_id, connected_entity, 'transaction', tx)
            elif tx.get('to_entity') == entity_id:
                connected_entity = tx.get('from_entity')
                if connected_entity:
                    direct_connections.add(connected_entity)
                    self._add_social_connection(connected_entity, entity_id, 'transaction', tx)
        ...
        clustering_coefficient = nx.clustering(self.social_graph, entity_id) if entity_id in self.social_graph else 0
        ...
        if high_risk_connections:
            risk_indicators.append(f"Connections to {len(high_risk_connections)} high-risk entities")
    ...
    def _add_social_connection(self, source: str, target: str,
                             connection_type: str, evidence: Dict[str, Any]):
        """Add or strengthen social connection"""
        if not self.social_graph.has_edge(source, target):
            self.social_graph.add_edge(source, target,
    ...
    async def _analyze_transaction_sequences(self, entity_id: str,
                                          context_data: Dict[str, Any]) -> Dict[str, Any]:
        """Analyze transaction sequences for suspicious patterns"""
        transactions = sorted(context_data.get('transactions', []),
    ...
    def _detect_sequence_patterns(self, transactions: List[Dict[str, Any]]) -> List[str]:
        ...
        structuring_count = sum(1 for amt in amounts if threshold * 0.8 <= amt < threshold)

        if structuring_count >= 3:
            patterns.append(f"Structuring pattern detected ({structuring_count} transactions)")

        # Check for round-trip transactions
        round_trips = self._detect_round_trip_patterns(transactions)
        if round_trips:
            patterns.append(f"Round-trip transactions detected ({len(round_trips)} patterns)")
    ...
    def _detect_round_trip_patterns(self, transactions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _detect_velocity_patterns(self, transactions: List[Dict[str, Any]]) -> List[str]:
        ...
        if max_hourly > avg_hourly * 3 and max_hourly > 10:
            patterns.append(f"Unusual transaction velocity: {max_hourly} transactions in one hour")

        return patterns

    def _detect_merchant_concentration(self, transactions: List[Dict[str, Any]]) -> List[str]:
        ...
        for merchant, count in merchant_counts.items():
            ...
            if percentage > 50:  # More than 50% of transactions with one merchant
                patterns.append(f"High concentration with merchant '{merchant}': {percentage:.1f}% of transactions")

        return patterns

    async def _analyze_device_fingerprints(self, entity_id: str,
                                        context_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if compromised_devices:
            risk_indicators.append(f"Known compromised devices: {len(compromised_devices)}")
    ...
    def _calculate_device_consistency(self, device_data: List[Dict[str, Any]]) -> float:
        ...
        fingerprints = [d.get('fingerprint') for d in device_data if d.get('fingerprint')]
    ...
    def _check_compromised_devices(self, device_data: List[Dict[str, Any]]) -> List[str]:
        """Check for known compromised devices"""
        # This would query a device reputation database
        return []

    async def _analyze_geospatial_patterns(self, entity_id: str,
                                        context_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _detect_geospatial_anomalies(self, locations: List[Dict[str, Any]]) -> List[str]:
        ...
        high_risk_transactions = sum(country_counts.get(country, 0) for country in high_risk_countries)

        if high_risk_transactions > 0:
            patterns.append(f"Transactions involving high-risk countries: {high_risk_transactions}")
    ...
    async def _analyze_temporal_patterns(self, entity_id: str,
                                      context_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _detect_temporal_anomalies(self, transactions: List[Dict[str, Any]]) -> List[str]:
        ...
        if time_diffs:
            ...
            burst_count = sum(1 for diff in time_diffs if diff < avg_interval * 0.1)  # Much faster than average

            if burst_count > len(time_diffs) * 0.3:  # More than 30% are bursts
                patterns.append(f"Transaction burst patterns detected: {burst_count} rapid transactions")
        ...
        unusual_hours = [hour for hour, count in hour_counts.items() if count > len(timestamps) * 0.4]
        if unusual_hours:
            patterns.append(f"Unusual transaction timing: concentrated in hours {unusual_hours}")

        return patterns

    def _combine_modality_scores(self, modality_results: Dict[str, Dict[str, Any]]) -> float:
    ...
    def _calculate_overall_confidence(self, modality_results: Dict[str, Dict[str, Any]]) -> float:
    ...
    async def _detect_fraud_patterns(self, entity_id: str,
        ...
        behavioral_risk = modality_results.get('behavioral_biometrics', {}).get('risk_score', 0)
        sequence_risk = modality_results.get('transaction_sequence', {}).get('risk_score', 0)
        ...
        device_risk = modality_results.get('device_fingerprint', {}).get('risk_score', 0)
        if behavioral_risk > 0.7 and device_risk > 0.6:
            patterns.append({
                ...
                'involved_modalities': ['behavioral_biometrics', 'device_fingerprint']
            })
    ...
    def _generate_recommendations(self, risk_score: float,
        ...
        for pattern in detected_patterns:
            ...
            if pattern_type == 'mule_network':
                recommendations.append("Investigate connections to other flagged accounts")
                ...
                recommendations.append("Review transaction patterns for SAR filing consideration")
