...
async def verify_intelligence_plugins():
    ...
    try:
        ...
        context = PluginContext(
            config={},
            services={'ai_service': MockAIService()}
        )
        ...
        match = any("Entity 'A' connected to 3" in i for i in linkage_result.get("insights", []))
        if match:
        ...
        evidence_case = {
            "evidence": [
                {"filename": "invoice.pdf", "content": "Payment to offshore shell company in Cayman"}
            ]
        }
