...
class TestCasesCRUD:
    ...
    def mock_db(self):
        ...
        with patch("app.routers.cases.db_service") as mock:
            ...
            mock.get_cases.return_value = [
                MagicMock(id="case-1", title="Case 1", status="open", priority="high"),
                MagicMock(
                    id="case-2",
                    title="Case 2",
            ...
            mock.get_case_by_id.return_value = MagicMock(
                id="test-case-123", title="Test Case", status="open"
            )

            # Mock update_case
            mock.update_case.return_value = MagicMock(
                id="test-case-123", status="investigating", risk_score=75.5
            )
    ...
    def test_update_case(self, mock_db):
        ...
        response = client.put(
            "/api/v1/cases/cases/test-case-123",
            json={"status": "investigating", "risk_score": 75.5},
        )
