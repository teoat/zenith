...
try:
    ...
    if os.getenv("TESTING") != "true" and os.path.exists("newrelic.ini"):
        newrelic.agent.initialize()
except (ImportError, Exception):
    pass  # New Relic not installed or config missing, skip
...
async def lifespan(app: FastAPI):
    ...
    from core.env_validator import validate_production_env

    if not validate_production_env():
        logger.warning("⚠️ SECURITY CHECK FAILED. But continuing for debug purposes...")
    else:
    ...
    for attempt in range(max_db_retries):
        ...
        try:
            ...
            if await initialize_cache():
                logger.info(
                    "✅ Redis Cache initialized",
                    extra={"service": "cache", "status": "initialized"},
                )
            else:
                logger.warning(
                    "⚠️ Redis Cache failed to initialize (operating without cache)",
                    extra={"service": "cache", "status": "failed"},
                )
        except (ZenithError, Exception) as e:
            logger.warning(f"Failed to initialize Redis Cache: {e}")

        # Phase 1.7: Async Database Manager Initialization
        logger.info(
            "Phase 1.7: Async Database Manager Initialization",
            extra={"startup_phase": 1.7},
        )
        try:
            from core.database.async_manager import get_async_db_manager

            async_db_manager = get_async_db_manager()
            await async_db_manager.initialize()
            logger.info(
                "✅ Async Database Manager initialized",
                extra={"service": "async_db", "status": "initialized"},
            )
        except (ZenithError, Exception) as e:
            logger.warning(f"Failed to initialize Async Database Manager: {e}")

        # Phase 1.8: Background Processor Initialization
        logger.info(
            "Phase 1.8: Background Processor Initialization",
            extra={"startup_phase": 1.8},
        )
        try:
            from core.jobs.background_processor import (
                example_evidence_processing_job,
                example_fraud_analysis_job,
            ...
            logger.info(
                ...
                extra={"service": "background_jobs", "status": "initialized"},
            )
        except (ZenithError, Exception) as e:
    ...
    try:
        ...
        try:
            from core.jobs.background_processor import get_background_processor
...
app.add_middleware(SecurityAuditMiddleware)  # Add Audit Middleware
...
if RATE_LIMITING_ENABLED:
    ...
    logger.warning("⚠️  Rate limiting is DISABLED (slowapi not installed)")
...
logger.info("✅ Prometheus metrics endpoint /metrics registered")

# Setup comprehensive API documentation with custom OpenAPI schema
app = setup_api_documentation(app)

# Initialize Interactive API Documentation Service
try:
    from app.services.docs.docs_service import docs_service

    docs_service.load_openapi_spec()
    logger.info(
        "✅ Interactive API Documentation service initialized",
        extra={"service": "docs", "status": "initialized"},
    )
except (ZenithError, Exception) as e:
    logger.warning(f"Failed to initialize API documentation service: {e}")

# Initialize Advanced Configuration Manager
try:
    from app.services.infrastructure.advanced_config_manager import config_manager

    # Start hot-reloading for configuration files
    config_manager.start_hot_reload()
    logger.info(
        "✅ Advanced Configuration Manager initialized",
        extra={"service": "config", "status": "initialized"},
    )
except (ZenithError, Exception) as e:
    logger.warning(f"Failed to initialize advanced configuration manager: {e}")
...
# Redis Test Endpoint (User Request)
@app.post("/api/redis-test")
async def test_redis_endpoint():
    """
    Fetch data from Redis key 'item'.
    """
    try:
        from app.services.infrastructure.redis_cluster import redis_cluster_manager
...
async def debug_env():
    """Debug endpoint to see environment variables (sanitized)"""
    return {
        "ENVIRONMENT": os.getenv("ENVIRONMENT"),
        "KV_URL_SET": bool(os.getenv("KV_URL")),
        "REDIS_URL_SET": bool(os.getenv("REDIS_URL")),
        "DATABASE_URL_SET": bool(os.getenv("DATABASE_URL")),
        "PYTHON_VERSION": os.getenv("PYTHON_VERSION"),
        "SECRET_KEY_LEN": len(os.getenv("SECRET_KEY", "")),
        "JWT_SECRET_KEY_LEN": len(os.getenv("JWT_SECRET_KEY", "")),
        "FIELD_ENCRYPTION_KEY_LEN": len(os.getenv("FIELD_ENCRYPTION_KEY", "")),
    }
