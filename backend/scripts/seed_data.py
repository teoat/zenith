...
from core.database import create_engine_and_session, Base
from core.database import Case, Transaction, Evidence, CaseNote, User, Team

# Sample data
SAMPLE_COMPANIES = [
    "TechStart Solutions", "Global Trade Corp", "Pacific Imports", 
    "Metro Construction", "Digital Services LLC", "Coastal Retail",
    "Summit Manufacturing", "Valley Logistics", "Urban Development",
    "Harbor Shipping Co"
]

SAMPLE_INVESTIGATORS = [
    "Sarah Chen", "Michael Rodriguez", "Emily Thompson", 
    "David Park", "Jessica Williams", "Robert Kim"
]
...
def generate_sample_cases(db: Session, users: list, count: int = 20):
    ...
    for i in range(count):
        ...
        case = Case(
            ...
            case_type=random.choice(["financial_fraud", "procurement_fraud", "embezzlement", "identity_theft"]),
    ...
    for case in cases:
        for i in range(random.randint(2, count_per_case)):
            transaction = Transaction(
                case_id=case.id,
                transaction_date=case.created_at + timedelta(days=random.randint(-30, 0)),
    ...
    for case in cases:
        for i in range(random.randint(2, 6)):
            note_content = random.choice(note_templates).format(
                indicator=random.choice(FRAUD_INDICATORS),
