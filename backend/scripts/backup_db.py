...
class BackupManager:
    ...
    def restore_backup(self, backup_path: str, verify_password: Optional[str] = None) -> bool:
        """
    ...
    def list_backups(self, backup_type: Optional[str] = None) -> list:
        ...
        """
        pattern = f"378x492_fraud_detection_{backup_type}_*.db" if backup_type else "378x492_fraud_detection_*.db"
        backups = []
    ...
    def get_backup_stats(self) -> dict:
        ...
        return {
            ...
            "total_size_mb": round(sum(b["size_bytes"] for b in all_backups) / 1024 / 1024, 2),
            "daily_backups": len([b for b in all_backups if "_daily_" in b["filename"]]),
            "weekly_backups": len([b for b in all_backups if "_weekly_" in b["filename"]]),
            "monthly_backups": len([b for b in all_backups if "_monthly_" in b["filename"]]),
