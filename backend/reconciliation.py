...
def reconcile_dataframes(df_a, df_b, date_tolerance_days=3):
    ...
    col_desc_a, col_desc_b = find_description_column(df_a), find_description_column(df_b)

    if not col_amt_a or not col_amt_b:
        return {"error": "Amount columns not found"}
    
    # Ensure dates are datetime
    if col_date_a: df_a[col_date_a] = pd.to_datetime(df_a[col_date_a], errors='coerce')
    if col_date_b: df_b[col_date_b] = pd.to_datetime(df_b[col_date_b], errors='coerce')
    ...
    for idx_a, row_a in df_a.iterrows():
        ...
        if col_date_a and col_date_b and not pd.isna(row_a[col_date_a]):
            ...
            if not exact_date_match.empty:
                ...
                matches.append({"record_a": row_a.to_dict(), "record_b": match_row_b.to_dict(), "type": "exact", "score": 100})
                continue
            ...
            if not date_window_match.empty:
                ...
                df_a.at[idx_a, '_matched'] = True
                df_b.at[match_row_b.name, '_matched'] = True
                matches.append({"record_a": row_a.to_dict(), "record_b": match_row_b.to_dict(), "type": "date_tolerance", "score": 95})
                continue
        ...
        for idx_a, row_a in df_a.iterrows():
            ...
            if not candidates.empty:
                ...
                if best_score > 70: # Fuzzy threshold
                    ...
                    matches.append({"record_a": row_a.to_dict(), "record_b": best_candidate.to_dict(), "type": "fuzzy", "score": best_score})

    # 4. Result Compilation
    unmatched_a = df_a[df_a['_matched'] == False].drop(columns=['_recon_id', '_matched']).fillna("").to_dict('records')
    unmatched_b = df_b[df_b['_matched'] == False].drop(columns=['_recon_id', '_matched']).fillna("").to_dict('records')
