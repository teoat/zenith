...
class TranslationValidator {
  ...
  loadTranslationFile(locale, namespace) {
    ...
    try {
      ...
      this.errors.push(`Failed to load ${locale}/${namespace}.json: ${error.message}`);
      return null;
    }
  }
  ...
  validateNamespace(namespace) {
    ...
    if (!enTranslation) {
      this.errors.push(`Reference translation (en/${namespace}.json) could not be loaded`);
      return;
    }

    const enKeys = this.getAllKeys(enTranslation);

    for (const locale of SUPPORTED_LOCALES.slice(1)) { // Skip 'en' as it's the reference
      ...
      if (missingKeys.length > 0) {
        this.errors.push(`Missing keys in ${locale}/${namespace}.json: ${missingKeys.join(', ')}`);
      }

      // Check for extra keys
      const extraKeys = localeKeys.filter(key => !enKeys.includes(key));
      if (extraKeys.length > 0) {
        this.warnings.push(`Extra keys in ${locale}/${namespace}.json: ${extraKeys.join(', ')}`);
      }
      ...
      if (emptyTranslations.length > 0) {
        this.warnings.push(`Empty translations in ${locale}/${namespace}.json: ${emptyTranslations.join(', ')}`);
      }
    }
  }
