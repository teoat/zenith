...
const EvidenceItem: React.FC<{
  ...
  return (
    <div
      ...
      `}
      draggable
      onDragStart={() => onDrag(evidence)}
      className="flex items-center gap-2 p-2 rounded border cursor-grab transition-all hover:bg-gray-50"
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
        if (e.key === 'Enter' || e.key === ' ') {
...
export const InvestigationCanvas: React.FC<InvestigationCanvasProps> = ({
  ...
  const handleExport = useCallback((format: 'json' | 'pdf' | 'image') => {
    if (onExport) {
      onExport(format);
    }
  }, [onExport]);
                ...
                <TabsContent value="connections">
                  <div className="space-y-2">
                    {relationships
                      ...
                      .map(relationship => {
                        const otherId = relationship.source === selectedEntity.id
                          ? relationship.target
                          : relationship.source;
