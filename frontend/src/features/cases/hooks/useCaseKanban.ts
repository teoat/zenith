...
export const useCaseKanban = (
  externalCases?: ApiCase[],
  ...
  const transformCase = useCallback(
    (c: ApiCase): Case => ({
      ...
      assignee: c.assigneeId
        ? { name: c.assigneeId.substring(0, 8), avatar: "U" }
        : undefined,
      dueDate: c.dueDate,
      tags: c.tags || [],
    }),
  ...
  useEffect(() => {
    if (externalCases) {
      const incoming = externalCases
        .filter((c: ApiCase) => c.status === "open" || c.status === "OPEN")
        .map(transformCase);

      const review = externalCases
        .filter(
          (c: ApiCase) =>
  ...
  useEffect(() => {
    ...
    const loadCases = async () => {
      try {
        ...
        const rawCases: ApiCase[] = Array.isArray(response)
          ? response
        ...
        const incoming = rawCases
          .filter((c: ApiCase) => c.status === "open" || c.status === "OPEN")
          .map(transformCase);

        const review = rawCases
          .filter(
            (c: ApiCase) =>
              c.status === "investigating" ||
              c.status === "INVESTIGATING" ||
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
        ...
        if (activeContainer && overContainer) {
          ...
          setItems((prev: KanbanState) => {
            ...
            return {
              ...prev,
              [activeContainer]: [
                ...prev[activeKey].filter(
                  (item: Case) => item.id !== active.id,
                ),
              ],
              [overContainer]: [
      ...
      const activeKey = activeContainer as keyof KanbanState;
      const activeIndex = items[activeKey].findIndex(
        (item: Case) => item.id === active.id,
      );
      const overIndex = items[activeKey].findIndex(
        (item: Case) => item.id === over?.id,
      );
