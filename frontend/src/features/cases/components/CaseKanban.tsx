...
import { CaseKanbanProps, Case } from "@/types/kanban";
import { useCaseKanban } from "../hooks/useCaseKanban";
import { KanbanColumn } from "./kanban/KanbanColumn";
import { KanbanCard } from "./kanban/KanbanCard";

const CaseKanban: React.FC<CaseKanbanProps> = React.memo(
  ({ cases: externalCases, onCaseClick }) => {
    const {
      activeId,
      loading,
      error,
    ...
    // Empty state
    const hasNoCases =
      !items.incoming.length && !items.review.length && !items.closed.length;
    if (hasNoCases) {
      return (
        <div className="flex items-center justify-center h-64">
          <div className="text-center max-w-md">
            ...
            <p className="text-slate-500 dark:text-slate-400 mb-4">
              Start your first fraud investigation by creating a case. Cases
              help you organize evidence, track suspects, and collaborate with
              your team.
            </p>
            <div className="text-sm text-slate-400 space-y-1">
              <p>
                💡 <strong>Tip:</strong> Use the "New Investigation" button
                above to get started
              </p>
              <p>
                🎯 <strong>Next:</strong> Upload evidence files to build your
                case
              </p>
            </div>
          </div>
...
CaseKanban.displayName = "CaseKanban";
