    ...
    >
      ...
      <div className="mb-3">
        <div className="flex justify-between items-center mb-1">
          ...
          <span
            className={`text-xs font-bold ${riskScore > 75 ? "text-red-600" : riskScore > 50 ? "text-amber-600" : "text-green-600"}`}
          >
            {riskScore}%
          </span>
        </div>
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {tags.slice(0, 3).map((tag, i) => (
            <span
              key={i}
              className="text-[10px] bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2 py-0.5 rounded"
            >
              {tag}
            </span>
          ))}
