...
const FacetedFilter: React.FC<FacetedFilterProps> = ({
  ...
  const [internalFilters, setInternalFilters] =
    useState<Record<string, FilterValue>>(selectedFilters);
  ...
  const handleResetFilters = () => {
    ...
    filterOptions.forEach((option) => {
      if (option.type === "slider" && option.min !== undefined) {
        resetFilters[option.id] = (option.defaultValue ??
          option.min) as FilterValue;
              ...
              {option.options?.map((opt) => (
                ...
                >
                  <input
                    ...
                    checked={
                      Array.isArray(internalFilters[option.id])
                        ? (internalFilters[option.id] as string[]).includes(
                            opt.value,
                          )
                        : false
                    }
                  ...
                  />
                  <span className="text-sm text-slate-600 dark:text-slate-300 group-hover:text-blue-600">
                    {opt.label}
                  </span>
                </label>
              ))}
            ...
            option.max !== undefined && (
              <>
                ...
                <Slider.Root
                  ...
                  step={1}
                  onValueChange={(value) =>
                    handleSliderChange(option.id, value)
                  }
                >
