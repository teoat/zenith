...
const CaseTable: React.FC<CaseTableProps> = React.memo(
  ({ cases, onOpenCase, isLoading }) => {
    const columns = useMemo((): Column<Case>[] => [
        ...
        },
        {
          ...
          render: (item: Case) => (
            <span
              className={`font-bold text-xs uppercase ${getPriorityColor(item.priority)}`}
            >
              {item.priority}
            </span>
          ),
    ...
    return (
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
        <DataGrid data={cases} columns={columns} isLoading={isLoading} />
      </div>
    );
  },
);
