...
export const useChaosEngineering = () => {
  ...
  const enableScenario = useCallback((scenarioId: string) => {
    chaosEngine.enableScenario(scenarioId);
    updateScenarios();
  }, [updateScenarios]);

  const disableScenario = useCallback((scenarioId: string) => {
    chaosEngine.disableScenario(scenarioId);
    updateScenarios();
  }, [updateScenarios]);

  const updateScenario = useCallback((scenarioId: string, updates: Partial<FailureScenario>) => {
