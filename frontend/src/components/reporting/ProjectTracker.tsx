...
const ProjectTracker = () => {
  ...
  useEffect(() => {
    const fetchTracker = async () => {
      try {
        const trackerData = await reportingService.getProjectTracker(
  ...
  if (loading)
    return (
      <div className="p-12 text-center text-slate-500">Loading tracker...</div>
    );
  if (!data) return null;
                ...
                <div
                  ...
                  ${
                    step.status === "complete"
                      ? "bg-green-500 border-green-100 text-white scale-110 shadow-sm"
                      : step.status === "delayed"
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800">
          ...
          <p className="text-sm text-slate-500 mb-4">
            ...
            <span className="text-orange-500 font-bold ml-1">
              Values {">"} 100 indicate likely overbilling.
            </span>
          </p>
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                Price/Unit vs Total Volume.{" "}
                <span className="text-rose-500">Red dots are outliers.</span>
              </p>
            </div>
              ...
              >
                ...
                <Scatter name="Vendors" data={VENDOR_OUTLIERS} fill="#3b82f6">
                  {VENDOR_OUTLIERS.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.outlier ? "#f43f5e" : "#3b82f6"}
                    />
                  ))}
                </Scatter>
              </ScatterChart>
