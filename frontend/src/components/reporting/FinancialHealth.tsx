  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const health = await reportingService.getFinancialHealth(
          activeProjectId || "CASE-001",
        );
        setData(health);
          ...
          <div className="flex justify-between items-start">
            ...
            <div
              className={`p-2.5 rounded-lg ${
                data.totalSpend / data.budget > 0.8
            ...
            {(data?.inflowCategories || []).map((cat) => (
              <div key={cat.id} className="group">
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span
                    className={`font-mono ${cat.amount < 0 ? "text-slate-500" : "text-green-600"}`}
                  >
                    {cat.amount > 0 ? "+" : ""}${(cat.amount / 1000).toFixed(0)}
                    k
                  </span>
                </div>
                <div className="w-full h-1.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${
                      cat.type === "excluded"
                        ? "bg-slate-300 dark:bg-slate-600 pattern-diagonal"
                        : cat.type === "inflow"
                          ? "bg-green-500"
                          : "bg-indigo-500"
                    }`}
                ...
                {cat.type === "excluded" && (
                  <p className="text-[10px] text-slate-400 mt-1 flex items-center gap-1">
                    <Activity size={10} /> Excluded from project calculation
                    (Internal Transfer)
                  </p>
                )}
              </div>
          ...
          <div className="flex-1 z-10">
            <div className="text-center mb-8">
              <p className="text-sm text-slate-400 mb-1">
                Projected Zero Funds Date
              </p>
              <p
                className={`text-2xl font-bold ${
                  Number(simulatorResults.runway) < 3
                    ? "text-red-400"
                    : "text-emerald-400"
                }`}
              >
