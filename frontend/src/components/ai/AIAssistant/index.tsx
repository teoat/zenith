...
export const AIAssistant: React.FC<AIAssistantProps> = ({
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello! I'm Frenly, your advanced fraud detection assistant. How can I help you investigate today?",
      timestamp: Date.now(),
    },
  ]);
  ...
  const handleActionClick = async (action: SuggestionAction) => {
    ...
    try {
      ...
      const result = (await aiService.performAction(
        action.endpoint,
        action.method || "POST",
        action.body,
      )) as any;
  ...
  const handleSend = async (message: string) => {
    ...
    try {
      ...
      const aiMsg: Message = {
        ...
        suggestions: response.data?.suggestions as
          | SuggestionAction[]
          | undefined,
      };
            ...
            try {
              ...
              await approvalService.createFromAISuggestion({
                ...
                details: {
                  ...
                  entityId:
                    suggestion.entityId || mergedContext?.project?.caseId || "",
                  payload: suggestion.payload,
                },
                reasoning: suggestion.reasoning || response.data?.response,
                confidence:
                  suggestion.confidence || response.data?.confidence || 0.8,
              });
            } catch (error) {
              secureLogger.error(
                "AI_INTELLIGENCE",
                "Failed to create approval action",
                {
      ...
      secureLogger.error(
        "AI_INTELLIGENCE",
        "Intelligence engine connection error",
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "assistant",
          content:
            "I'm having trouble connecting to the intelligence engine right now. Please try again.",
          timestamp: Date.now(),
          persona: activePersona,
        },
      ]);
    ...
    <div>
      ...
      {isOpen && (
        <div
          ...
          className="fixed bottom-24 right-6 w-[400px] h-[600px] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl z-50 flex flex-col border border-slate-200 dark:border-slate-800 overflow-hidden"
        >
