...
import type { CodeIssue, TestSuggestion, AIAnalysisResponse, AnalysisResult } from '@/types/code-review';
...
def get_user_data(user_id):
    ...
    return db.execute(query)
      ...
      const issuesByCategory = aiIssues.reduce((acc, issue) => {
        acc[issue.category] = (acc[issue.category] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);

      const issuesBySeverity = aiIssues.reduce((acc, issue) => {
        acc[issue.severity] = (acc[issue.severity] || 0) + 1;
        return acc;
      }, {} as Record<string, number>);

      const linesOfCode = sampleCode.split('\n').length;
      const maintainabilityIndex = Math.max(0, 100 - (issuesBySeverity['critical'] * 10 + issuesBySeverity['high'] * 5));
      ...
      const mockSuggestions: TestSuggestion[] = [
        {
          test_type: "unit_test",
          code_snippet: "def test_get_user_data():\n    # Test cases for user data retrieval\n    assert get_user_data(1) == \"expected_data\"",
          target_function: "get_user_data",
          justification: "Add unit tests to prevent SQL injection and validate inputs"
        }
      ];
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-lg">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Code Review Dashboard</h2>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div className="bg-white p-6 border rounded-lg">
                <h3 className="text-lg font-semibold text-blue-600">{displayResult.total_issues}</h3>
