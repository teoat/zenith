    ...
    factors: [
      {
        ...
        weight: 0.35,
        direction: "negative",
        description: "340% increase in last 24h",
      },
      {
        name: "Geographic Dispersion",
        weight: 0.25,
        direction: "negative",
        description: "Transactions from 12 countries",
      },
      {
        name: "Structuring Pattern",
        weight: 0.2,
        direction: "negative",
        description: "Multiple sub-10k transactions",
      },
      {
        name: "Network Risk",
        weight: 0.2,
        direction: "negative",
        description: "Connected to 3 flagged entities",
      },
    ],
    ...
    factors: [
      {
        ...
        weight: 0.4,
        direction: "negative",
        description: "Round-trip flows detected",
      },
      {
        name: "Dormancy Pattern",
        weight: 0.3,
        direction: "negative",
        description: "Sudden reactivation",
      },
      {
        name: "Ownership Opacity",
        weight: 0.3,
        direction: "negative",
        description: "Nominee directors only",
      },
    ],
    ...
    factors: [
      {
        ...
        weight: 0.45,
        direction: "negative",
        description: "Price discrepancies detected",
      },
      {
  ...
  return (
    <div className="risk-score-ring" style={{ width: size, height: size }}>
      ...
      <div className="ring-content">
        <span className="ring-value" style={{ color: getColor(score) }}>
          {score}
        </span>
      ...
      <div className="prediction-header">
        <div className="entity-info">
          <div className="entity-icon">
            {getEntityIcon(prediction.entityType)}
          </div>
          <div>
        ...
        {prediction.factors.slice(0, 3).map((factor, idx) => (
          <div key={idx} className="factor-item">
            <div className="factor-header">
              ...
              <Badge
                variant={
                  factor.direction === "negative" ? "destructive" : "default"
                }
      ...
      <div className="prediction-footer">
        ...
        <div className="predicted-patterns">
          {prediction.predictedPatterns.map((pattern, idx) => (
            <Badge key={idx} variant="outline" className="pattern-badge">
              {pattern}
            </Badge>
          ))}
        </div>
...
export const PredictiveDashboard: React.FC<PredictiveDashboardProps> = ({
  ...
  const stats = useMemo(() => {
    ...
    const increasing = predictions.filter(
      (p) => p.riskTrend === "increasing",
    ).length;
    const avgConfidence = Math.round(
      predictions.reduce((s, p) => s + p.confidence, 0) / predictions.length,
    );
    const avgRisk = Math.round(
      predictions.reduce((s, p) => s + p.riskScore, 0) / predictions.length,
    );
      ...
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Select
              value={timeHorizon}
              onValueChange={(v) => setTimeHorizon(v as typeof timeHorizon)}
            >
          ...
          <div className="stat-card risk">
            ...
            <div className="stat-details">
              ...
              <span className="stat-description">
                Across {stats.total} monitored entities
              </span>
            </div>
          </div>
