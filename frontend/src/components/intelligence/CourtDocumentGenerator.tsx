...
// Document templates
const documentTemplates: Record<DocumentType, {
  name: string;
  description: string;
...
export const CourtDocumentGenerator: React.FC<CourtDocumentGeneratorProps> = ({
  ...
  const addExhibit = useCallback(() => {
    ...
    setExhibits(prev => [...prev, { id: `ex-${Date.now()}`, label: `Exhibit ${label}`, description: '' }]);
  }, [exhibits.length]);
      ...
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-lg">Court Document Generator</CardTitle>
        ...
        {!previewMode ? (
          <>
                ...
                {sections.map(section => (
                  <div key={section.id} className="document-section">
                    ...
                    <Textarea
                      ...
                      placeholder={sectionTemplates[section.id]?.placeholder || 'Enter content...'}
                      rows={4}
                    />
                  </div>
                ))}
            ...
            <div className="actions-section">
              <Progress value={completionPercentage} className="completion-progress" />
              <div className="action-buttons">
                <Button variant="outline" onClick={() => setPreviewMode(true)} disabled={sections.length === 0}>
                  <Eye className="w-4 h-4 mr-1" />
                  Preview
                </Button>
                <Button onClick={handleGenerate} disabled={isGenerating || completionPercentage < 50}>
                  {isGenerating ? (
                    <>
          ...
          <div className="preview-container">
            <div className="preview-header">
              ...
              <div className="preview-actions">
                <Button variant="outline" onClick={() => onExport?.(generatedDocument!, 'pdf')}>
                  <Download className="w-4 h-4 mr-1" />
                  Export PDF
                </Button>
                <Button variant="outline" onClick={() => onExport?.(generatedDocument!, 'docx')}>
                  <Download className="w-4 h-4 mr-1" />
                  Export DOCX
                </Button>
              </div>
              ...
              <div className="legal-document-preview">
                ...
                {sections.map(section => (
                  <div key={section.id} className="preview-section">
                    ...
                    <div className="section-content">{section.content || '[Content pending]'}</div>
                  </div>
                ))}
                ...
                {exhibits.length > 0 && (
                  <div className="exhibits-list">
                    <h2>EXHIBITS</h2>
                    {exhibits.map(ex => (
                      <p key={ex.id}><strong>{ex.label}:</strong> {ex.description}</p>
                    ))}
                  </div>
                )}
                ...
                <div className="signature-block">
                  ...
                  <p>[Attorney Name]<br/>Attorney for [Party]</p>
