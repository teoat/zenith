  ...
  const [agents, setAgents] = useState<AgentStatus[]>([
    {
      ...
      status: "idle",
      message: "Waiting for queries...",
      icon: <Globe size={14} />,
    },
    {
      id: "2",
      agentName: "Fraud Analyst",
      status: "idle",
    ...
    },
    {
      ...
      status: "idle",
      message: "Ready to score evidence...",
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setAgents((prev) => {
        ...
        const statuses: ("idle" | "busy" | "searching" | "analyzing")[] = [
          "idle",
          "busy",
          "searching",
          "analyzing",
      ...
      <div className="space-y-2">
        {agents.map((agent) => (
          <div key={agent.id} className="flex items-start gap-3">
            <div
              className={`mt-0.5 p-1 rounded bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 ${agent.status !== "idle" ? "text-blue-500" : "text-slate-400"}`}
            >
              {agent.icon}
            </div>
