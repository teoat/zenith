      ...
      for (let i = 0; i < filesToAnalyze.length; i++) {
        ...
        try {
          ...
          try {
            ...
            secureLogger.warn(
              `Detailed analysis failed for ${file.name}, using upload metadata`,
              analysisErr,
            );
        ...
        <div className="flex items-center gap-2">
          {STEPS.map((step, idx) => (
            <div key={step.id} className="flex items-center">
              ...
              >
                <span
                  className={`flex items-center justify-center w-6 h-6 rounded-full text-xs box-content border ${
                    idx === currentStep
          ...
          <div className="h-full flex flex-col items-center justify-center max-w-2xl mx-auto space-y-8">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-2">
                Upload Bank Statements & Receipts
              </h2>
        ...
        {currentStep === 1 && (
          <div className="h-full flex flex-col">
            ...
            {processingResults[activeFileIndex] && (
              <DataMapping
                sourceColumns={
                  processingResults[activeFileIndex].detectedHeaders || []
                }
                previewData={
                  processingResults[activeFileIndex].rawPreviewData || []
                }
          ...
          <div className="h-full flex flex-col max-w-4xl mx-auto w-full">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 mb-6 pr-2">
              {processingResults.map((res, idx) => (
                <div
                  key={idx}
                  className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 flex justify-between items-center"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        Mapped {Object.keys(res.mappingConfig || {}).length}{" "}
                        fields
                      </p>
                    </div>
                  </div>
                  ...
                  try {
                    for (const res of processingResults) {
                      if (res.savedId && res.mappingConfig) {
                        await api.ingestMappedData(
                          res.savedId,
                          res.mappingConfig,
                        );
                        updateProcessingResult(processingResults.indexOf(res), {
                          status: "completed",
                        });
                        successCount++;
                      }
                    ...
                    addToast(
                      `Successfully ingested ${successCount} files`,
                      "success",
                    );
