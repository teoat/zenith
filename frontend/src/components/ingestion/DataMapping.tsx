...
export const DataMapping: React.FC<DataMappingProps> = ({
  ...
  const isValid = SYSTEM_FIELDS.filter((f) => f.required).every(
    (f) => mapping[f.id],
  );
                ...
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700">
                    {SYSTEM_FIELDS.map((f) => (
                      <th
                        key={f.id}
                        className="text-left py-2 px-2 font-medium text-slate-500"
                      >
                        {f.label}
                      </th>
                    ))}
                    ...
                    <tr key={idx}>
                      {SYSTEM_FIELDS.map((f) => {
                        ...
                        return (
                          <td
                            key={f.id}
                            className="py-2 px-2 text-slate-700 dark:text-slate-300 truncate max-w-[100px]"
                          >
                            {val}
                          </td>
                        );
