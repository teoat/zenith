...
const AIReasoningTab: React.FC<AIReasoningTabProps> = ({ alert }) => {
  ...
  const reasoning = alert.ai_reasoning || {
    summary:
      "This transaction deviates significantly from the established pattern for this entity.",
    confidence: 0.87,
    indicators: [
      {
        type: "amount_anomaly",
        score: 0.92,
        desc: "Amount is 3x higher than 30-day average",
      },
      {
        type: "velocity_risk",
        score: 0.78,
        desc: "2nd transaction in 10 minutes",
      },
      {
        type: "geo_risk",
        score: 0.45,
        desc: "IP address from high-risk jurisdiction",
      },
    ],
  };
      ...
      <div>
        ...
        <div className="space-y-3">
          {reasoning.indicators.map((ind, i) => (
            <div
              key={i}
              className="flex items-start gap-3 p-3 bg-white dark:bg-slate-900 rounded border border-slate-100 dark:border-slate-800"
            >
              <div
                className={`mt-1 ${ind.score > 0.8 ? "text-red-500" : ind.score > 0.5 ? "text-amber-500" : "text-blue-500"}`}
              >
                <ShieldAlert size={16} />
              </div>
