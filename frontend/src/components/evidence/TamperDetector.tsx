...
export const TamperDetector: React.FC<TamperDetectorProps> = ({
  evidence,
  className = "",
}) => {
  ...
  if (!evidence) {
    return (
      <div
        className={`p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-800 text-center ${className}`}
      >
        <ShieldCheck className="mx-auto text-slate-400 mb-2" size={24} />
        <p className="text-sm text-slate-500">
          Select a file to run tamper analysis
        </p>
      </div>
    );
  }
  ...
  // Mock Analysis Logic
  const isSuspicious =
    evidence.fileType === "pdf" && evidence.sizeBytes > 5000000; // Mock rule
  const metadata = {
    ...
    ? [
        {
          ...
          severity: "high",
          message:
            "Metadata indicates editing software (Photoshop) used on invoice.",
        },
        {
          id: 2,
          severity: "medium",
          message: "Creation date matches modification date.",
        },
      ]
    : [];
      ...
      {flags.length > 0 && (
        <div className="space-y-2">
          {flags.map((flag) => (
            <div
              key={flag.id}
              className="p-3 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/50 rounded-md flex gap-3 items-start"
            >
              <AlertTriangle
                size={16}
      ...
      <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-md p-3">
        ...
        <div className="space-y-2 text-xs">
          <div className="flex justify-between">
            ...
            <span
              className={`font-mono ${isSuspicious ? "text-red-500 font-bold" : "text-slate-700 dark:text-slate-300"}`}
            >
              {metadata.software}
            </span>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <span
              className="text-slate-700 dark:text-slate-300 font-mono truncate max-w-[150px]"
              title="e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
            >
              e3b0c442...b855
            </span>
          </div>
