...
export const MensReaAnalyzer: React.FC<MensReaAnalyzerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"INTENT" | "KNOWLEDGE" | "MOTIVE">(
    "INTENT",
  );
  ...
  const [indicators] = useState<IntentIndicator[]>([
    {
      ...
      type: "KNOWLEDGE",
      description:
        "Subject accessed sensitive admin panel 3 times prior to incident",
      confidence: 85,
      evidenceIds: ["log_123"],
    },
    {
  ...
  const getIndicatorsByType = (type: string) =>
    indicators.filter((i) => i.type === type);
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 text-sm">
              Automated intent pattern recognition and legal element mapping
            </p>
          </div>
          ...
          {[
            {
              ...
              icon: <Target size={20} />,
              label: "Knowledge",
              desc: "Awareness of Facts",
            },
            {
              id: "INTENT",
              icon: <ArrowRight size={20} />,
              label: "Intent",
              desc: "Purpose to Commit",
            },
            {
              id: "MOTIVE",
              icon: <Scale size={20} />,
              label: "Motive",
              desc: "Reason for Action",
            },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() =>
                setActiveTab(tab.id as "INTENT" | "KNOWLEDGE" | "MOTIVE")
              }
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {indicator.evidenceIds.map((eid) => (
                      <div
                        key={eid}
                        className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-xs text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700"
                      >
