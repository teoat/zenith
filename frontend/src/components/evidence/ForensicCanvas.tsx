...
import PdfViewer from "@/components/forensics/PdfViewer";
...
export const ForensicCanvas: React.FC<ForensicCanvasProps> = ({
  ...
  const enableAdvancedViewing =
    import.meta.env.VITE_ENABLE_ADVANCED_FORENSIC !== "false";
  const useSimplePdfViewer =
    import.meta.env.VITE_USE_SIMPLE_PDF_VIEWER === "true";

  const [viewMode, setViewMode] = useState<"visual" | "hex" | "metadata">(
    "visual",
  );
      ...
      <div className="flex flex-col h-full bg-slate-950">
        <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
          ...
          <h3 className="text-lg font-medium mb-2">
            Advanced Viewing Disabled
          </h3>
          <p className="text-sm text-center max-w-md">
            Forensic analysis features have been disabled to improve
            performance. Basic file information is still available in the
            evidence panel.
          </p>
          {fileUrl && (
              ...
              {evidence?.fileType === "pdf" || fileUrl?.endsWith(".pdf") ? (
                <PdfViewer
                  ...
                  onHighlight={(highlight) => {
                    if (evidence?.id) {
                      api
                        .saveHighlight(evidence.id, highlight)
                        .catch((error) => {
                          secureLogger.error(
                            "FORENSIC_CANVAS",
        ...
        {viewMode === "hex" && (
          <div className="absolute inset-0 overflow-auto p-4 font-mono text-xs text-green-500 bg-slate-950 leading-relaxed whitespace-pre-wrap select-text">
                ...
                {Array.from({ length: 20 }).map((_, i) => (
                  <div key={i} className="flex gap-2">
                    <span className="hover:bg-slate-800 cursor-pointer">
                      25 50 44 46 2d 31 2e 35 0d 0a 25 b5 b5 b5 b5 0d
                    </span>
            ...
            <p className="mt-4 text-slate-500 italic text-center">
              — End of Preview (First 320 bytes) —
            </p>
          </div>
        )}
        ...
        {viewMode === "metadata" && (
          <div className="absolute inset-0 overflow-auto p-8">
            <div className="max-w-2xl mx-auto bg-slate-900 rounded-lg border border-slate-800 p-6 font-mono text-sm shadow-xl">
              <h3 className="text-slate-400 border-b border-slate-800 pb-2 mb-4 uppercase text-xs tracking-wider">
                File Metadata
              </h3>
              <div className="grid grid-cols-[150px_1fr] gap-y-2">
                ...
                <span className="text-slate-200">
                  Adobe Photoshop CC 2023 (Macintosh)
                </span>
                ...
                <span className="text-slate-200">
                  xmp.did:48291048-2231-4122-8821
                </span>
              </div>
            </div>
          </div>
