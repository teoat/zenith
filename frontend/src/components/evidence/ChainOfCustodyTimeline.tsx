      ...
      <CardContent>
        <div className="space-y-4">
          {evidence.chainOfCustody.map((event) => (
            <div
              key={event.id}
              className="border-l-4 border-blue-500 pl-4 py-2 bg-slate-50 dark:bg-slate-800/50 rounded-r-lg"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2">
                  <Badge
                    variant="outline"
                    className="capitalize border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300"
                  >
                    {event.action}
                  </Badge>
        ...
        <div className="mt-6 p-4 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
          <div className="flex items-center space-x-2 mb-2">
            <Shield
              className={`h-5 w-5 ${evidence.integrityVerified ? "text-green-600" : "text-red-600"}`}
            />
