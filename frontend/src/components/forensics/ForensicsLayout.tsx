...
const MetricCard = ({ label, value, icon: Icon }: { label: string; value: number; icon: React.ElementType }) => (
...
const ForensicsLayout: React.FC<ForensicsLayoutProps> = memo(() => {
  ...
  const selectedEvidence = evidenceData?.items.find((item: EvidenceItem) => item.id === selectedEvidenceId) || null;
  ...
  const stats = {
    ...
    flagged: evidenceData?.items.filter((i: any) => i.riskLevel === 'high' || i.tags?.includes('flagged')).length || 0,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    <PageErrorBoundary>
      <div className="forensics-layout h-full flex flex-col bg-slate-950 text-slate-200">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 border-b border-slate-800 bg-slate-900/50">
          <MetricCard label="Total Evidence" value={stats.total} icon={FileText} />
          <MetricCard label="High Risk" value={stats.flagged} icon={ShieldAlert} />
          <MetricCard label="Processed" value={stats.processed} icon={Activity} />
          <MetricCard label="Pending Analysis" value={stats.pending} icon={Clock} />
        </div>
