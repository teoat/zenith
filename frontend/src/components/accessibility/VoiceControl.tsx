...
const VoiceControl: React.FC = () => {
  ...
  useEffect(() => {
    if (!supported) {
      secureLogger.warn(
        "VOICE_CONTROL",
        "Web Speech API not supported in this browser. Enabling simulated mode.",
      );
    }
  }, [supported]);
  ...
  const handleCommand = (command: string) => {
    ...
    if (lowerCmd.includes("dashboard") || lowerCmd.includes("alerts")) {
      ...
      lowerCmd.includes("investigation") ||
      lowerCmd.includes("cases")
    ) {
  ...
  const toggleListening = () => {
    if (listening) {
      ...
      setTimeout(() => {
        ...
        secureLogger.info(
          "VOICE_CONTROL",
          `Recognized command: ${mockCommand}`,
        );
