...
const generateMockHypotheses = (): Hypothesis[] => [
  {
    ...
    description:
      "The subject is using multiple shell companies to layer funds through a structured network of transactions designed to obscure the origin of funds.",
    status: "validated",
    confidence: 85,
  ...
  },
  {
    ...
    description:
      "Nominee directors and complex ownership structures are being used to hide the true beneficial owner of multiple entities.",
    status: "testing",
    confidence: 72,
  ...
  },
  {
    ...
    description:
      "Bank employees may be complicit in enabling unusual transaction patterns without proper reporting.",
    status: "proposed",
    confidence: 45,
    createdBy: "Sarah Reviewer",
    createdAt: new Date("2024-12-05"),
    votes: [{ userId: "u1", vote: "support" }],
    evidence: [
  ...
  },
  {
    ...
    description:
      "The primary motivation for the scheme is to evade domestic taxation through offshore structures.",
    status: "inconclusive",
    confidence: 55,
    createdBy: "Jane Analyst",
...
const HypothesisCard: React.FC<{
  ...
  const supportCount = hypothesis.votes.filter(
    (v) => v.vote === "support",
  ).length;
  const opposeCount = hypothesis.votes.filter(
    (v) => v.vote === "oppose",
  ).length;
  const supportPercentage =
    hypothesis.votes.length > 0
  ...
  const opposingEvidence = hypothesis.evidence.filter(
    (e) => !e.supports,
  ).length;
...
export const HypothesisBoard: React.FC<HypothesisBoardProps> = ({
  ...
  const [hypotheses, setHypotheses] = useState<Hypothesis[]>(
    propHypotheses || generateMockHypotheses(),
  );
  const [showCreateDialog, setShowCreateDialog] = useState(false);
  const [newHypothesis, setNewHypothesis] = useState<Partial<Hypothesis>>({});
  const [_selectedHypothesis, setSelectedHypothesis] =
  ...
  const handleVote = useCallback(
    (hypothesisId: string, vote: "support" | "oppose") => {
      setHypotheses((prev) =>
        prev.map((h) => {
          if (h.id === hypothesisId) {
            return {
              ...h,
              votes: [
                ...h.votes.filter((v) => v.userId !== "currentUser"),
                { userId: "currentUser", vote },
              ],
            };
          }
          return h;
        ...
        <div className="hypothesis-columns">
          ...
          <div className="hypothesis-column">
            <div className="column-header proposed">
              ...
              <Badge variant="outline">
                {groupedHypotheses.proposed.length}
              </Badge>
            </div>
          ...
          <div className="hypothesis-column">
            <div className="column-header testing">
              ...
              <Badge variant="outline">
                {groupedHypotheses.testing.length}
              </Badge>
            </div>
            <ScrollArea className="column-content">
          ...
          <div className="hypothesis-column">
            <div className="column-header validated">
              ...
              <span>Validated</span>
              <Badge variant="outline">
                {groupedHypotheses.validated.length}
              </Badge>
            </div>
            <ScrollArea className="column-content">
