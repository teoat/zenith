...
interface IntentIndicator {
  id: string;
  category: 'knowledge' | 'premeditation' | 'concealment' | 'pattern' | 'communication';
  title: string;
  description: string;
  ...
  const indicators: IntentIndicator[] = [
    {
      ...
      description: 'Subject attended multiple compliance training sessions and acknowledged understanding.',
      strength: 'strong',
      confidence: 92,
      evidence: ['Training records', 'Signed acknowledgments', 'Email confirmations'],
      suggestedQuestions: ['When was the last training attended?', 'Were there compliance reminders?']
    },
    {
      id: 'ind2',
      category: 'premeditation',
      title: 'Advance Planning',
      description: 'Evidence shows structured planning over 3+ months including entity creation.',
      strength: 'strong',
      confidence: 88,
      evidence: ['Entity registration dates', 'Pre-arranged account openings', 'Calendar entries'],
      suggestedQuestions: ['What was the timeline for entity setup?', 'Were there consultants involved?']
    },
    {
      id: 'ind3',
      category: 'concealment',
      title: 'Active Concealment Efforts',
      description: 'Multiple shell companies used to obscure beneficial ownership.',
      strength: 'strong',
      confidence: 95,
      evidence: ['Corporate structure', 'Nominee directors', 'PO Box addresses'],
      suggestedQuestions: ['Who are the ultimate beneficial owners?', 'Why use nominee directors?']
    },
    {
      id: 'ind4',
      category: 'pattern',
      title: 'Repeated Conduct',
      description: 'Similar transaction patterns observed across multiple quarters.',
      strength: 'moderate',
      confidence: 75,
      evidence: ['Transaction analysis', 'Quarterly reports'],
      suggestedQuestions: ['Is this consistent with business operations?', 'Were there legitimate explanations?']
    },
    {
      id: 'ind5',
      category: 'communication',
      title: 'Incriminating Communications',
      description: 'Emails discussing "keeping things off the books" and using code words.',
      strength: 'strong',
      confidence: 98,
      evidence: ['Email thread #A234', 'Chat logs', 'Voice recordings'],
      suggestedQuestions: ['What is the context of these communications?', 'Who were the recipients?']
    }
  ];

  const timeline: IntentTimeline[] = [
    { date: new Date('2024-01-15'), event: 'First shell company registered', significance: 'high', category: 'premeditation' },
    { date: new Date('2024-02-20'), event: 'Offshore account opened', significance: 'high', category: 'concealment' },
    { date: new Date('2024-03-10'), event: 'First suspicious transfer', significance: 'medium', category: 'pattern' },
    { date: new Date('2024-04-05'), event: 'Compliance inquiry received', significance: 'medium', category: 'knowledge' },
    { date: new Date('2024-04-12'), event: 'Evasive response to inquiry', significance: 'high', category: 'concealment' },
    { date: new Date('2024-05-20'), event: 'Internal email about "keeping quiet"', significance: 'high', category: 'communication' },
  ];
          ...
          <svg viewBox="0 0 36 36">
            ...
            <path
              d="M18 2.0845
                ...
                a 15.9155 15.9155 0 0 1 0 -31.831"
              fill="none"
              stroke={indicator.confidence >= 80 ? '#22c55e' : indicator.confidence >= 60 ? '#f59e0b' : '#64748b'}
              strokeWidth="3"
      ...
      <div className="indicator-evidence">
        ...
        <div className="evidence-tags">
          {indicator.evidence.map((ev, i) => (
            <Badge key={i} variant="outline" className="evidence-tag">{ev}</Badge>
          ))}
        </div>
      </div>
...
export const MensReaAnalyzer: React.FC<MensReaAnalyzerProps> = ({
  ...
  const [_selectedIndicator, setSelectedIndicator] = useState<IntentIndicator | null>(null);

  // Calculate overall intent score
  const intentScore = useMemo(() => {
    const weights = { strong: 3, moderate: 2, weak: 1 };
    const totalWeight = indicators.reduce((sum, ind) => sum + weights[ind.strength] * (ind.confidence / 100), 0);
  ...
  const legalElements: LegalElement[] = useMemo(() => [
    {
      ...
      satisfied: indicators.some(i => i.category === 'knowledge' && i.strength === 'strong'),
      evidence: indicators.filter(i => i.category === 'knowledge').flatMap(i => i.evidence)
    },
    {
      element: 'Knowledge of Wrongdoing',
      satisfied: indicators.some(i => (i.category === 'knowledge' || i.category === 'communication') && i.confidence >= 80),
      evidence: indicators.filter(i => i.category === 'knowledge' || i.category === 'communication').flatMap(i => i.evidence)
    },
    {
      element: 'Deliberate Action',
      satisfied: indicators.some(i => i.category === 'premeditation' && i.strength !== 'weak'),
      evidence: indicators.filter(i => i.category === 'premeditation').flatMap(i => i.evidence)
    },
    {
      element: 'Attempt to Conceal',
      satisfied: indicators.some(i => i.category === 'concealment'),
      evidence: indicators.filter(i => i.category === 'concealment').flatMap(i => i.evidence)
    }
  ], [indicators]);

  const handleIndicatorClick = useCallback((indicator: IntentIndicator) => {
            ...
            <div className="intent-score-ring">
              <svg viewBox="0 0 100 100">
                ...
                <circle
                  ...
                  r="40"
                  fill="none"
                  stroke={intentScore >= 70 ? '#ef4444' : intentScore >= 50 ? '#f59e0b' : '#22c55e'}
                  strokeWidth="8"
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="legal" className="mt-4">
            <div className="legal-elements">
              {legalElements.map((element, idx) => (
                <div key={idx} className={`legal-element ${element.satisfied ? 'satisfied' : 'unsatisfied'}`}>
                  ...
                  <Badge variant={element.satisfied ? 'default' : 'destructive'}>
              ...
              <div className="legal-summary">
                ...
                <div className="summary-content">
                  ...
                  <p>
                    {legalElements.filter(e => e.satisfied).length} of {legalElements.length} elements established.
          ...
          <TabsContent value="timeline" className="mt-4">
            <div className="intent-timeline">
              {timeline.sort((a, b) => a.date.getTime() - b.date.getTime()).map((event, idx) => (
                <div key={idx} className={`timeline-item ${event.significance}`}>
