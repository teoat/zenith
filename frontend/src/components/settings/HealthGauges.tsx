...
const HealthGauges: React.FC = () => {
  ...
  const gauges: Gauge[] = [
    ...
    },
    {
      label: "Memory",
      value: metrics
        ? parseFloat((metrics.memory_used_mb / 1024).toFixed(1))
        : 0,
    ...
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
        ...
        <span className="flex items-center gap-1.5 text-xs text-green-600 font-medium">
          <span
            className={`w-2 h-2 rounded-full ${loading ? "bg-slate-400" : "bg-green-500 animate-pulse"}`}
          ></span>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 mb-3">
                <div
                  className="p-1.5 rounded bg-white dark:bg-slate-700"
              ...
              <div className="relative w-16 h-16 mx-auto mb-2">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span
                    className={`text-sm font-bold ${getStatus(gauge.value, gauge.max)}`}
                  >
                    {gauge.value}
                  </span>
                </div>
