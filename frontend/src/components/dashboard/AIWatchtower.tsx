    ...
    const fetchInsights = async () => {
      try {
        ...
        setInsights(rawInsights.map(i => ({
          id: i.id,
          title: i.message.substring(0, 30) + (i.message.length > 30 ? '...' : ''),
          message: i.message,
            ...
            >
              ...
              <div className="flex justify-between items-start mb-2 pr-6">
                <span className={`text-xs font-bold px-2 py-0.5 rounded uppercase tracking-wider ${
                  insight.type === 'pattern' ? 'bg-red-500/20 text-red-300' : 
                  insight.type === 'anomaly' ? 'bg-amber-500/20 text-amber-300' : 
                  'bg-blue-500/20 text-blue-300'
                }`}>
                  {insight.type}
                </span>
                <span className="text-xs font-mono text-emerald-400">{insight.confidence}% Conf</span>
              </div>
