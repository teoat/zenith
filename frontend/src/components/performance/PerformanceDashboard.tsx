...
const PerformanceDashboard: React.FC<PerformanceDashboardProps> = memo(
  ({ showInDevelopment = true }) => {
    ...
    useEffect(() => {
      ...
      return () =>
        window.removeEventListener("togglePerformanceDashboard", handleToggle);
    }, []);
    ...
    const slowComponents = Object.values(metrics).filter(
      (m: any) => m.isSlow,
    ).length;
    const avgRenderTime =
      totalComponents > 0
        ? Object.values(metrics).reduce(
            (sum: number, m: any) => sum + m.averageRenderTime,
            0,
          ) / totalComponents
        : 0;
        ...
        <div className="p-3 space-y-2">
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div>
              ...
              <div
                className={`font-mono ${slowComponents > 0 ? "text-red-400" : "text-green-400"}`}
              >
                {slowComponents}
              </div>
            </div>
            <div className="col-span-2">
              ...
              <div
                className={`font-mono ${avgRenderTime > 16 ? "text-yellow-400" : "text-green-400"}`}
              >
              ...
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {Object.entries(metrics)
                  ...
                  .map(([name, m]: [string, any]) => (
                    <div
                      key={name}
                      className="text-xs bg-red-900/20 rounded px-2 py-1"
                    >
                      <div className="font-mono text-red-400 truncate">
                        {name}
                      </div>
                      <div className="text-red-300">
                        {m.slowestRenderTime.toFixed(1)}ms ({m.renderCount}{" "}
                        renders)
                      </div>
                    </div>
                  ))}
