...
const RoleSelection: React.FC = () => {
        ...
        if (mounted)
          setAvailable(
            res.roles && res.roles.length > 0
              ? res.roles
              : ["analyst", "investigator", "admin", "viewer"],
          );
      })
      .catch(() => {
        if (mounted)
          setAvailable(["analyst", "investigator", "admin", "viewer"]);
      })
      .finally(() => {
  ...
  const getRoleDescription = (r: Role) => {
    switch (r) {
      ...
      case "investigator":
        return "Deep dive into cases and evidence chains";
      case "analyst":
        return "Triage alerts and monitor dashboards";
      case "viewer":
    ...
    <section data-testid="role-selection" className="space-y-4">
      <div className="flex flex-col gap-1 mb-4">
        ...
        <p className="text-sm text-muted-foreground">
          Customize your workspace for your primary tasks.
        </p>
      </div>
