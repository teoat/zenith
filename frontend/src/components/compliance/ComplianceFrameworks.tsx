  ...
  return (
    <Card className="border-slate-200 dark:border-slate-800 shadow-sm">
      <CardHeader>
        ...
        <CardDescription>
          Live compliance status across multi-jurisdictional regulatory
          frameworks
        </CardDescription>
      </CardHeader>
      <CardContent>
          ...
          {frameworks.map((framework, index) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant={
                    framework.status === "compliant" ? "default" : "secondary"
                  }
                  className={`${
                ...
                <p className="text-[10px] font-bold text-slate-500 flex items-center gap-1">
                  {framework.status === "compliant" ? (
                    <span className="flex items-center gap-1 text-emerald-500 uppercase">
                      <Sparkles className="w-3 h-3 text-amber-400" /> Fully
                      Compliant
                    </span>
                  ) : (
