...
const NodeInspector: React.FC<NodeInspectorProps> = memo(
  ({ node, onClose, onEdit, onDelete }) => {
    if (!node) return null;
    ...
    const getTypeIcon = () => {
      switch (node.type) {
        ...
        case "company":
          return <Building size={20} className="text-amber-500" />;
        case "account":
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              <button
                type="button"
                className="flex w-full items-center gap-2 p-2 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg transition-colors group text-left"
              >
                <div className="w-8 h-8 bg-red-100 dark:bg-red-900/30 rounded flex items-center justify-center text-xs font-bold text-red-600">
                  PDF
                </div>
