...
interface InvestigationWizardProps {
  onComplete?: (investigation: InvestigationData) => void;
  onCancel?: () => void;
}
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (e.target.checked) {
                  ...
                  onChange({
                    ...data,
                    evidenceTypes: types.filter((t: string) => t !== type),
                  });
                }
              }}
              className="mr-2"
...
const ReviewStep: React.FC<{ data: InvestigationData }> = ({ data }) => (
  <div className="space-y-4">
    <div className="bg-gray-50 p-4 rounded-lg">
      <h3 className="font-medium text-gray-900 mb-2">Investigation Summary</h3>
      <dl className="space-y-1 text-sm">
        <div>
          <dt className="font-medium inline">Title:</dt>{" "}
          <dd className="inline ml-1">{data.title || "Not specified"}</dd>
        </div>
        <div>
        ...
        <div>
          <dt className="font-medium inline">Scope:</dt>{" "}
          <dd className="inline ml-1 capitalize">{data.scope || "local"}</dd>
        </div>
        <div>
          <dt className="font-medium inline">Evidence Types:</dt>{" "}
          <dd className="inline ml-1">
            {data.evidenceTypes?.join(", ") || "None selected"}
          </dd>
        </div>
      </dl>
    </div>
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Follow the steps below to set up your investigation properly.
        </p>
      </div>
