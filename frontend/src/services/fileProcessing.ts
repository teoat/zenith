  ...
  async processCaseImportFiles(files: File[]): Promise<FileProcessingResult> {
    ...
    for (const file of files) {
      try {
        ...
        const errorMessage = `Failed to process ${file.name}: ${err instanceof Error ? err.message : 'Unknown error'}`;
    ...
    if (result.processedCases.length > 0) {
      try {
        ...
        result.errors.push(`Failed to create cases in backend: ${err instanceof Error ? err.message : 'Unknown error'}`);
        result.success = false;
      }
    }
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        try {
          ...
          result.errors.push(`Error processing row ${i + 1}: ${err instanceof Error ? err.message : 'Unknown error'}`);
        }
      }
    } catch (err) {
      result.errors.push(`Failed to read CSV file: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
    ...
    try {
      ...
      for (const caseData of casesData) {
        try {
          ...
          result.errors.push(`Error processing case: ${err instanceof Error ? err.message : 'Unknown error'}`);
        }
      }
    } catch (err) {
      result.errors.push(`Failed to parse JSON file: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
    ...
    for (let i = 0; i < cases.length; i += batchSize) {
      ...
      for (const caseData of batch) {
        try {
          ...
          errorReporting.reportApiError(err, 'createCase', 'POST');
          throw new Error(`Failed to create case "${caseData.title}": ${err instanceof Error ? err.message : 'Unknown error'}`);
        }
      }
