...
class FileProcessingService {
  ...
  async processCaseImportFiles(files: File[]): Promise<FileProcessingResult> {
    ...
    for (const file of files) {
      try {
        ...
        if (fileResult.errors.length > 0) {
          result.errors.push(
            ...fileResult.errors.map((err) => `${file.name}: ${err}`),
          );
        }
      } catch (error: unknown) {
    ...
    if (result.processedCases.length > 0) {
      try {
        ...
        result.errors.push(
          `Failed to create cases in backend: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
        result.success = false;
      }
    }
  ...
  private async processSingleFile(
    file: File,
  ): Promise<{ cases: ProcessedCase[]; errors: string[] }> {
        ...
        try {
          ...
          if (values.length === headers.length) {
            const caseData: ProcessedCase = {
              ...
              priority:
                (values[headers.indexOf("priority")] as unknown as
                  | "LOW"
                  | "MEDIUM"
                  | "HIGH"
                  | "CRITICAL") || "MEDIUM",
      ...
      for (const caseData of casesData) {
        try {
          const processedCase: ProcessedCase = {
            ...
            priority: (caseData.priority || "MEDIUM").toUpperCase() as
              | "LOW"
              | "MEDIUM"
              | "HIGH"
              | "CRITICAL",
          ...
          result.errors.push(
            `Error processing case: ${error instanceof Error ? error.message : "Unknown error"}`,
          );
    ...
    for (let i = 0; i < cases.length; i += batchSize) {
      ...
      for (const caseData of batch) {
        try {
          ...
          throw new Error(
            `Failed to create case "${caseData.title}": ${error instanceof Error ? error.message : "Unknown error"}`,
          );
        }
      }
