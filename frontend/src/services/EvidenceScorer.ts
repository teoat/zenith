...
export class EvidenceScorer {
  ...
  scoreEvidence(
    ...
    const admissibilityRating = this.determineAdmissibility(
      components,
      overallScore,
    );
    ...
    const legalNotes = this.generateLegalNotes(
      evidence,
      admissibilityRating,
      context?.legalJurisdiction,
    );
  ...
  scoreBatch(
    ...
    const scores = evidenceList.map((ev) =>
      this.scoreEvidence(ev, {
        ...context,
        relatedEvidence: evidenceList.filter((e) => e.id !== ev.id),
      }),
    );
  ...
  private scoreReliability(evidence: EvidenceItem): ScoreComponent {
    ...
    if (["transaction", "digital", "document"].includes(evidence.type)) {
      factors.push({
        ...
        weight: 15,
        description:
          "Evidence type is generally more reliable and less subject to interpretation",
      });
      score += 15;
    }
    ...
    // Recency
    const ageInDays =
      (Date.now() - evidence.dateObtained.getTime()) / (1000 * 60 * 60 * 24);
    if (ageInDays < 30) {
      factors.push({
      ...
      factors.push({
        ...
        weight: 5,
        description:
          "Evidence may be subject to degradation or obsolescence concerns",
      });
      score -= 5;
    }
  ...
  private scoreRelevance(
    ...
    if (["transaction", "document"].includes(evidence.type)) {
      factors.push({
        ...
        weight: 10,
        description:
          "Evidence type typically directly relevant to financial investigations",
      });
      score += 10;
    }
  ...
  private scoreChainOfCustody(evidence: EvidenceItem): ScoreComponent {
    ...
    if (custody.length >= 3) {
      factors.push({
        ...
        weight: 25,
        description:
          "Evidence has comprehensive chain of custody documentation",
      });
      score += 30;
  ...
  private scoreCorroboration(
    ...
    const sameTypeCount = relatedEvidence.filter(
      (e) => e.type === evidence.type,
    ).length;
    if (sameTypeCount >= 2) {
      factors.push({
    ...
    const crossTypeCount = relatedEvidence.filter(
      (e) => e.type !== evidence.type,
    ).length;
    if (crossTypeCount >= 1) {
      factors.push({
        ...
        weight: 15,
        description:
          "Evidence from different sources/types available for cross-validation",
      });
      score += 10;
    }
  ...
  private determineAdmissibility(
    components: ScoreComponent[],
    overallScore: number,
  ): EvidenceScore["admissibilityRating"] {
    const chainOfCustody = components.find(
      (c) => c.category === "chain_of_custody",
    );
  ...
  private identifyStrengths(
    ...
    if (evidence.type === "transaction") {
      strengths.push({
        type: "direct",
        description:
          "Transaction records provide direct evidence of financial activity",
        confidence: 90,
      });
    }
  ...
  private identifyWeaknesses(components: ScoreComponent[]): string[] {
    ...
    for (const component of components) {
      if (component.score < 60) {
        weaknesses.push(
          `Weak ${component.category.replace("_", " ")} (${component.score}/100)`,
        );
      }
  ...
  private generateRecommendations(
    ...
    if ((chainOfCustody?.score || 0) < 70) {
      recommendations.push(
        "Document and verify chain of custody for all evidence handlers",
      );
    }

    const corroboration = components.find(
      (c) => c.category === "corroboration",
    );
    if ((corroboration?.score || 0) < 70) {
      recommendations.push(
        "Seek additional corroborating evidence from independent sources",
      );
    }

    if (weaknesses.length > 0) {
      recommendations.push(
        "Address identified weaknesses before presenting evidence in legal proceedings",
      );
    }
  ...
  private generateLegalNotes(
    ...
    if (admissibility === "inadmissible") {
      notes.push(
        "WARNING: Evidence may face significant admissibility challenges",
      );
    }

    if (evidence.type === "digital") {
      notes.push(
        "Digital evidence should be authenticated per FRE 901(b)(9) or equivalent",
      );
    }

    if (evidence.type === "testimonial") {
      notes.push(
        "Testimonial evidence subject to hearsay rules - confirm exceptions apply",
      );
    }
