...
const GRAPH_TUTORIAL_STEPS: Step[] = [
  {
    ...
    description:
      "Visualize complex relationships between entities in a 3D space.",
  },
  {
    title: "Navigation",
    description:
  ...
  },
  {
    ...
    description:
      "The left sidebar lists all nodes. Click one to focus on it in the graph.",
  },
  {
    title: "Snapshots",
    description:
      'Save the current state of your investigation using the "Save Snapshot" button.',
  },
  {
    title: "Node Details",
    description:
      "Click any node in the graph to view detailed properties and metadata.",
  },
];
  ...
  if (error)
    return (
      <div className="p-8 text-red-500">
        {error.message || "Failed to load investigation data"}
      </div>
    );
          ...
          <div className="flex justify-between items-center px-6 h-full">
            ...
            <div className="flex gap-3 items-center">
              ...
              <div className="flex -space-x-2 mr-2 border-r border-slate-800 pr-4">
                <div
                  className="w-8 h-8 rounded-full bg-blue-600 border-2 border-slate-900 flex items-center justify-center text-[10px] font-bold text-white shadow-sm ring-2 ring-slate-900 z-30"
                  title="You"
                >
                  ME
                </div>
                <div
                  className="w-8 h-8 rounded-full bg-purple-600 border-2 border-slate-900 flex items-center justify-center text-[10px] font-bold text-white shadow-sm ring-2 ring-slate-900 z-20"
                  title="Alice Analyst"
                >
                  AA
                </div>
                <div
                  className="w-8 h-8 rounded-full bg-emerald-600 border-2 border-slate-900 flex items-center justify-center text-[10px] font-bold text-white shadow-sm ring-2 ring-slate-900 z-10"
                  title="Bob Boss"
                >
                  BB
                </div>
              ...
              <div className="hidden xl:flex items-center gap-2 mr-2 bg-slate-800/50 p-1.5 rounded-full border border-slate-700/50">
                ...
                <span className="text-[10px] text-blue-400 font-mono pr-1">
                  LIVE
                </span>
              </div>
              ...
              <AccessibleButton variant="secondary" onClick={handleReset}>
                <RotateCcw className="w-4 h-4 mr-2" />
                Reset
              </AccessibleButton>

              <AccessibleButton variant="primary" onClick={handleSaveSnapshot}>
