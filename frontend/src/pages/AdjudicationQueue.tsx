...
const AdjudicationQueue = () => {
  ...
  const processDecision = useCallback(
    async (id: string, decision: "approved" | "rejected" | "escalated") => {
      try {
        ...
        const nextAlert =
          alerts.find(
            (a, idx) => idx > currentIndex && a.status === "pending",
          ) ||
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowDown") {
        ...
        if (e.key === "a" || e.key === "A")
          processDecision(selectedId, "approved");
        if (e.key === "r" || e.key === "R")
          processDecision(selectedId, "rejected");
        if (e.key === "e" || e.key === "E")
          processDecision(selectedId, "escalated");
      }
    },
    [alerts, selectedId, selectedAlert, processDecision],
  ); // Added processDecision dependency
