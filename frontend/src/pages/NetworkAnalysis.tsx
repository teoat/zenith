...
const NetworkGraph = React.lazy(
  () => import("../components/visualizations/NetworkGraph"),
);

const NetworkAnalysis: React.FC = () => {
  const [graphData, setGraphData] = useState<NetworkGraphData | undefined>(
    undefined,
  );
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            Maximize detection by visualizing complex relationships between
            entities, accounts, and transactions.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-lg shadow border border-gray-200 dark:border-slate-800">
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="text-xs text-slate-500">
            ...
            {graphData?.links.length || 0} Connections • Mode:{" "}
            {mode.toUpperCase()}
          </div>
        </div>
