...
const AgentDrafts: React.FC = () => {
  const [drafts, setDrafts] = useState<AgentDraft[]>([
    {
      ...
      description: 'Auto-populated SAR reflecting unusual wire activity from high-risk jurisdiction.',
      type: 'SAR',
      impact: 'high',
      confidence: 0.94,
      reasoning: 'Transaction velocity exceeds baseline by 400% in a 12-hour window.',
    ...
    },
    {
      ...
      description: 'Administrative hold on account ending in *4429 due to suspected fraud sweep.',
      type: 'FREEZE',
      impact: 'critical',
      confidence: 0.88,
      reasoning: 'Multiple failed MFA attempts followed by attempt to transfer total balance.',
    ...
    },
    {
      ...
      description: 'Deep scan for secondary connections between Subject A and known associates.',
      type: 'INVESTIGATION',
      impact: 'medium',
      confidence: 0.72,
      reasoning: 'Shared IP address detected across three unrelated corporate accounts.',
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'SAR': return <FileText className="w-5 h-5" />;
      case 'FREEZE': return <ShieldAlert className="w-5 h-5" />;
      case 'CLEANUP': return <Trash2 className="w-5 h-5" />;
      case 'INVESTIGATION': return <Search className="w-5 h-5" />;
  ...
  const handleAction = (id: string, action: 'apply' | 'reject') => {
    ...
    setDrafts(prev => prev.map(d => 
      d.id === id ? { ...d, status: action === 'apply' ? 'applied' : 'rejected' } : d
    ));
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-8 pt-12">
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            <div className="flex items-center gap-2 text-indigo-500 mb-2">
              ...
              <span className="text-sm font-semibold tracking-wider uppercase">AI Agent Intelligence</span>
            </div>
            <h1 className="text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">
              Agent Drafts & Proposals
            </h1>
            <p className="mt-2 text-slate-600 dark:text-slate-400 max-w-2xl text-lg">
              Review and adjudicate autonomous agent proposals. Actions require human oversight before execution.
            </p>
          </div>
        ...
        <div className="grid gap-6">
          {drafts.filter(d => d.status === 'pending' && d.title.toLowerCase().includes(searchTerm.toLowerCase())).map((draft) => (
            <div 
              key={draft.id}
            ...
            >
              <div className="flex flex-col lg:flex-row">
                ...
                <div className={`w-1.5 shrink-0 ${
                  draft.impact === 'critical' ? 'bg-red-500' : 
                  draft.impact === 'high' ? 'bg-orange-500' : 
                  'bg-indigo-500'
                }`} />
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-4">
                        <div className={`p-3 rounded-xl ${getImpactColor(draft.impact)}`}>
                          {getIcon(draft.type)}
                        </div>
                        <div>
                    ...
                    <div className="bg-slate-50 dark:bg-slate-950/50 rounded-xl p-4 border border-slate-200/50 dark:border-slate-800/50">
                       <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 block mb-1">AI Reasoning</span>
        ...
        {drafts.filter(d => d.status === 'pending').length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800">
             ...
             <p className="text-slate-500 dark:text-slate-400 text-center max-w-xs">
               No pending agent drafts at this time. All autonomous proposals have been processed.
             </p>
          </div>
        )}
      </div>
