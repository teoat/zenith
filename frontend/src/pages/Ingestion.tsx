...
import FacetedFilter from "@/features/cases/components/FacetedFilter";
...
const Ingestion: React.FC = () => {
  const { filters, setFilters } = useIngestionStore();
  ...
  React.useEffect(() => {
    ...
    const checkEdgeAI = async () => {
      ...
      const { edgeInferenceService } = await import(
        "../services/edge/inferenceService"
      );
        ...
        <header className="page-header mb-6 flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400">
              Upload, map, and process evidence files
            </p>
          </div>
          {edgeReady && (
