...
interface EvidenceListProps {
  ...
  onFileSelect: (item: EvidenceItem) => void;
  onAddEvidence: () => void;
}
...
interface EvidenceViewerProps {
  ...
  activeTab: 'EVIDENCE' | 'MENS_REA' | 'HYPOTHESIS';
...
const EvidenceList: React.FC<EvidenceListProps> = ({
  ...
  const displayEvidence = useMemo(() =>
    isSemanticMode ? semanticResults : evidence,
    [isSemanticMode, semanticResults, evidence]
  );
  ...
  return (
    <div className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col shrink-0">
      ...
      <div className="flex-1 overflow-y-auto">
        {displayEvidence.map(fileOrResult => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 p-2 rounded-lg text-left transition-all border ${
                selectedEvidence?.id === file.id ? 'bg-blue-900/20 text-blue-200 border-blue-500/30 shadow-sm' : 'border-transparent hover:bg-slate-800 text-slate-400 hover:text-slate-200'
              }`}
            >
              <div className={`p-2 rounded bg-slate-800 shrink-0 ${selectedEvidence?.id === file.id ? 'bg-blue-900/50 text-blue-400' : ''}`}>
...
const EvidenceViewer: React.FC<EvidenceViewerProps> = ({
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
      ...
      onPanelResize(Math.max(250, Math.min(600, newWidth)));
    };
  ...
  return (
    <div className="flex-1 bg-slate-950 relative flex flex-col min-w-0 overflow-hidden">
      {activeTab === 'EVIDENCE' && (
        <div className="flex h-full w-full">
          <div className="flex-1 relative">
            <Suspense fallback={<LoadingState type="skeleton" context="component" size="lg" />}>
          ...
          <div
            ...
            onKeyDown={(e) => {
              if (e.key === 'ArrowLeft') {
                e.preventDefault();
                onPanelResize(Math.max(200, rightPanelWidth - 20));
              } else if (e.key === 'ArrowRight') {
                e.preventDefault();
                onPanelResize(Math.min(600, rightPanelWidth + 20));
              }
            }}
            role="separator"
      ...
      {activeTab === 'MENS_REA' && (
        <Suspense fallback={<LoadingState type="skeleton" context="component" size="lg" />}>
          <MensReaAnalyzer evidence={selectedEvidence ? [selectedEvidence] : []} />
        </Suspense>
      )}

      {activeTab === 'HYPOTHESIS' && (
        <Suspense fallback={<LoadingState type="skeleton" context="component" size="lg" />}>
          <HypothesisBoard evidence={selectedEvidence ? [selectedEvidence] : []} />
        </Suspense>
      )}
    </div>
  );
...
const Forensics: React.FC<ForensicsProps> = () => {
  ...
  const [isSemanticMode] = useState(false);
  ...
  // Fetch evidence with React Query pattern
  const fetchEvidence = useCallback(async (p: number = 1, q: string = '') => {
    try {
      ...
      secureLogger.error('Failed to fetch evidence:', err);
      setError('Failed to load evidence files. Please check connection.');
      addToast('Failed to load evidence', 'error');
    } finally {
      setLoading(false);
    }
  }, [selectedEvidence, addToast]);
  ...
  const handleSemanticSearch = useCallback(async (query: string) => {
    try {
      ...
      secureLogger.error('Semantic search failed:', err);
  ...
  const handleFileSelect = useCallback((item: EvidenceItem) => {
    ...
    return (
      <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-slate-950 text-slate-200">
        <div className="bg-slate-900 border-b border-slate-800 px-4 h-12 flex items-center gap-4 shrink-0">
          <button className="text-sm font-medium px-4 py-3 border-b-2 border-blue-500 text-blue-400">
            Evidence Board
          </button>
        </div>
        <div className="flex flex-1 overflow-hidden">
          <div className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col shrink-0">
            ...
            <div className="flex-1 flex items-center justify-center">
              <LoadingState type="spinner" context="data" size="md" text="Loading evidence..." />
            </div>
          </div>
          <div className="flex-1 bg-slate-950 relative flex flex-col min-w-0 overflow-hidden">
            <div className="flex-1 flex items-center justify-center">
              <LoadingState type="shimmer" context="component" size="lg" text="Preparing forensic workspace..." />
            </div>
          </div>
        </div>
      </div>
    ...
    <>
      ...
      <div className="flex flex-1 overflow-hidden">
        <EvidenceList
          ...
          onAddEvidence={() => setIsUploadWizardOpen(true)}
        />
        ...
        <EvidenceViewer
          ...
          activeTab={activeTab}
          onPanelResize={setRightPanelWidth}
        />
      </div>
