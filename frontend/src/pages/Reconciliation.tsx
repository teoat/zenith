...
const Reconciliation = () => {
  ...
  const bankItems = reconciliationItems.filter(
    (i) =>
      (i.source.includes("Bank") || i.source === "Bank Feed") &&
      i.status !== "matched" &&
      i.status !== "discrepancy",
  );
  const ledgerItems = reconciliationItems.filter(
    (i) =>
      (i.source.includes("Ledger") || i.source === "Internal Ledger") &&
      i.status !== "matched" &&
      i.status !== "discrepancy",
  );
  const exceptionItems = reconciliationItems.filter(
      ...
      <header className="flex-shrink-0 mb-4 flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Match transactions and resolve discrepancies
          </p>
        </div>
        ...
        <div className="flex-1 grid grid-cols-12 gap-6 min-h-0">
          ...
          <div className="col-span-9 flex flex-col min-h-0">
            <MatchCanvas
              ...
              onMatch={(sourceId, _targetId) => {
                void reconcileItem(sourceId);
              }}
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-green-600">
                    {Math.round(
                      (reconciliationItems.filter((i) => i.status === "matched")
                        .length /
          ...
          >
            ...
            <div className="space-y-2 mb-6 text-sm text-slate-700 dark:text-slate-300">
              <p>
                ...
                <span className="font-mono">{selectedItem.transactionId}</span>
              </p>
              <p>
