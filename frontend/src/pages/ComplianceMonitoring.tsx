...
} from "@/services/complianceMonitoring";
...
const ComplianceTrends = React.lazy(() =>
  import("@/components/compliance/ComplianceTrends").then((module) => ({
    default: module.ComplianceTrends,
  })),
);
    ...
    try {
      ...
      const healthChecksData: HealthCheck[] = [
        ...
        },
        {
          ...
          status:
            healthData.overall_compliance_score > 80 ? "healthy" : "warning",
          message: `Compliance score: ${healthData.overall_compliance_score}%`,
        },
        {
          name: "Alert System",
          status:
            healthData.open_security_incidents === 0 ? "healthy" : "warning",
  ...
  if (error) {
    return (
      <div className="p-6">
        <Alert
          variant="destructive"
          className="rounded-2xl shadow-lg border-red-200"
        >
          <AlertTriangle className="h-5 w-5" />
          <AlertTitle className="text-lg font-bold">
            Monitoring Subsystem Error
          </AlertTitle>
          <AlertDescription>
            {error}. Please contact devops or verify API connectivity.
          </AlertDescription>
        </Alert>
      </div>
    );
    ...
    <div className="space-y-8 p-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2 max-w-2xl text-lg">
            Live telemetry from regulatory engines, audit subsystems, and active
            security perimeter.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button
            size="lg"
            className="rounded-xl bg-indigo-600 hover:bg-indigo-500 shadow-lg shadow-indigo-600/20"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          {
            label: "Uptime",
            value: `${dashboard?.system_health.uptime || 0}%`,
            icon: Server,
          ...
          {
            label: "Avg Latency",
            value: `${dashboard?.system_health.response_time || 0}ms`,
            icon: Zap,
            color: "text-blue-500",
            bg: "bg-blue-50",
          },
          {
            label: "Error Margin",
            value: `${((dashboard?.system_health.error_rate || 0) * 100).toFixed(2)}%`,
            icon: AlertTriangle,
            color: "text-red-500",
            bg: "bg-red-50",
          },
          {
            ...
            value: dashboard?.system_health.active_users || 0,
            icon: Activity,
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <CardTitle className="text-lg">
                    Latency & Error Performance
                  </CardTitle>
                  <CardDescription>
                    KPI analysis for compliance engine throughput
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                <div className="space-y-4">
                  ...
                  <Progress
                    value={Math.min(
                      (dashboard?.performance_metrics.api_response_time || 0) /
                        5,
                      100,
                    )}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between font-bold">
                    ...
                    <span className="text-indigo-600">
                      {dashboard?.performance_metrics.database_query_time || 0}
                      ms
                    </span>
                  </div>
                  <Progress
                    value={Math.min(
                      (dashboard?.performance_metrics.database_query_time ||
                        0) / 2,
                      100,
                    )}
                    className="h-2.5 rounded-full"
                ...
                <div className="space-y-4">
                  ...
                  <Progress
                    value={
                      (dashboard?.performance_metrics.error_rate || 0) * 1000
                    }
        ...
        <div className="space-y-8">
          ...
          <div className="p-6 bg-indigo-600 rounded-3xl shadow-xl shadow-indigo-600/20 text-white relative overflow-hidden group">
            ...
            <h4 className="text-xl font-extrabold mb-2 relative z-10">
              Governance Report
            </h4>
            <p className="text-sm text-indigo-100 mb-6 relative z-10">
              Export a real-time snapshot of the compliance state for regulatory
              auditors.
            </p>
