...
export function useWebSocketQueue(options: UseWebSocketQueueOptions = {}) {
  const { maxItems = 20, fallbackToSimulation = true } = options;
  ...
  useEffect(() => {
    ...
    const unsubscribe = addListener((data) => {
      if (data.type === "queue_item") {
        const id =
          typeof data.id === "string" ? data.id : Date.now().toString();
        const type = (
          typeof data.item_type === "string" &&
          ["review", "alert", "system"].includes(data.item_type)
            ? data.item_type
            : "alert"
        ) as QueueItem["type"];
        const msg =
          typeof data.message === "string" ? data.message : "Unknown message";
        ...
        const priority = (
          ...
          ["high", "medium", "low"].includes(data.priority)
            ? data.priority
            : "medium"
        ) as QueueItem["priority"];
        const caseId =
          typeof data.case_id === "string" ? data.case_id : undefined;
        ...
        if (Array.isArray(data.items)) {
          const items = (data.items as Record<string, unknown>[]).map(
            (item) => ({
              ...
              type: (["review", "alert", "system"].includes(item.type as string)
                ? item.type
                : "alert") as QueueItem["type"],
  ...
  useEffect(() => {
    ...
    secureLogger.info(
      "WEBSOCKET",
      "LiveQueue simulation mode active (No WebSocket)",
    );
        ...
        return [
          {
            id: "1",
            type: "review",
            msg: "Case #492 requires adjudication",
            time: "2m ago",
            priority: "high",
          },
          {
            id: "2",
            type: "alert",
            msg: "Structuring pattern detected (User A)",
            time: "5m ago",
            priority: "high",
          },
          {
          ...
          },
          {
            id: "4",
            type: "review",
            msg: "KYC Verification pending for New Corp",
            time: "15m ago",
            priority: "medium",
          },
        ];
    ...
    const interval = setInterval(() => {
      const newActivities: QueueItem[] = [
        ...
        },
        {
          ...
          type: "review",
          msg: "Large transaction flagged ($45,000)",
          time: "just now",
          priority: "high",
        },
        {
      ...
      const randomMsg =
        newActivities[Math.floor(secureRandom.random() * newActivities.length)];
