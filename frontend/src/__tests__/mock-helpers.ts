 ...
 * ```
 */
export const createMockPromise = <T>(): jest.MockedFunction<
  () => Promise<T>
> => {
...
export const mockApiMethod = <TData>() => {
  const mock = jest.fn() as unknown as MockedFunction<
    () => Promise<{
      data?: TData;
      success: boolean;
...
export const mockCollectionResponse = <T>() => {
  const mock = jest.fn() as unknown as jest.MockedFunction<
    () => Promise<{
      success: true;
...
export const createAsyncMockReturn = <TArgs extends any[], TReturn>(
  ...
  const mock = jest.fn() as jest.MockedFunction<
...
export const createMockServices = () => {
  ...
  return {
    authService: {
      login: mockFactory({ login: jest.fn() }, "login", {
