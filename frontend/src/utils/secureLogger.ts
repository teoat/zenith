...
export class SecureLogger {
  ...
  private outputToConsole(entry: LogEntry): void {
    ...
    const sanitizedMessage = entry.sanitized
      ? entry.message
      : "[UNSANITIZED] " + entry.message;
  ...
  private async sendToSecureService(entry: LogEntry): Promise<void> {
    try {
      ...
      console.error(
        "[SecureLogger] Failed to send log to remote service:",
        error,
      );
  ...
  debug(arg1: unknown, arg2?: unknown, arg3?: unknown): void {
    const { category, message, metadata } = this.parseArgs(
      "debug",
      arg1,
      arg2,
      arg3,
  ...
  private parseArgs(
    ...
    arg2?: unknown,
    arg3?: unknown,
  ): { category: string; message: string; metadata?: Record<string, unknown> } {
  ...
  auditLog(
    ...
    this.info("AUDIT", `User action: ${action} on ${resource}`, {
      ...
      userId: String(this.sanitizeData(userId)),
      changes: changes
        ? (this.sanitizeData(changes) as Record<string, unknown>)
        : undefined,
