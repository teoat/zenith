...
async def lifespan(app: FastAPI):
    ...
    if os.getenv("ENVIRONMENT", "development").lower() == "production":
        logger.info("🔒 Performing production security checks...")
        if not validate_production_env():
            logger.critical("❌ SECURITY CHECK FAILED. Aborting startup.")
            raise RuntimeError(
                "Production security validation failed. Check environment variables."
            )
