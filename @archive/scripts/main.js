...
let secureIPC;
let sessionManager;
let backendApiKey = require('crypto').randomBytes(32).toString('hex');
...
const createPyProc = () => {
  ...
  if (require('fs').existsSync(script)) {
    pyProc = require('child_process').spawn(script, [], {
      stdio: 'pipe',
      env: { ...process.env, PORT: '8000', BACKEND_API_KEY: backendApiKey }
    });
...
function setupIPCHandlers(secureIPC, sessionManager) {
  // Database operations
