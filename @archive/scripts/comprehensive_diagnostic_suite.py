...
class ComprehensiveDiagnosticSuite:
    ...
    async def diagnose_backend_area(self):
        ...
        if backend_path.exists():
            ...
            backend_metrics["file_structure"]["python_files"] = len(
                list(backend_path.rglob("*.py"))
            )
            backend_metrics["file_structure"]["test_files"] = len(
                list(backend_path.rglob("test_*.py"))
            )
        else:
        ...
        services = {
            ...
            "compliance_service": [
                "backend/app/services/workflow/compliance/compliance_service.py"
            ],
        ...
        for service_name, paths in services.items():
            ...
            for path_str in paths:
                if (self.repo_root / path_str).exists():
                    backend_metrics["services_status"][service_name] = (
                        "healthy (modular)"
                    )
                    found = True
                    break
        ...
        if routes_path.exists():
            for route_file in routes_path.rglob("*.py"):
                try:
                    ...
                    endpoint_count += len(
                        re.findall(r"@router\.(get|post|put|delete|patch)", content)
                    )
                except OSError:
                    continue
            backend_metrics["api_endpoints"]["total_endpoints"] = endpoint_count
            backend_metrics["api_endpoints"]["route_files"] = len(
                list(routes_path.rglob("*.py"))
            )
        ...
        try:
            ...
            backend_metrics["resource_usage"]["disk_check"] = f"error: {e!s}"

        self.results["areas"]["backend"] = backend_metrics
        healthy_services = sum(
            1
            for s in backend_metrics["services_status"].values()
            if "healthy" in str(s) or "exists" in str(s)
        )
        print(
            f"   ✅ Backend diagnosis complete - {healthy_services}/{len(backend_metrics['services_status'])} services detected"
        )

        if backend_metrics["api_endpoints"]:
            print(
                f"   📍 Found {backend_metrics['api_endpoints']['total_endpoints']} API endpoints"
            )
    ...
    async def diagnose_frontend_area(self):
        ...
        if frontend_path.exists():
            try:
                ...
                frontend_metrics["build_status"] = (
                    "built" if build_exists else "needs_build"
                )
        ...
        print(
            f"   ✅ Frontend diagnosis complete - {frontend_metrics['component_count']} components found"
        )
    ...
    async def diagnose_database_area(self):
        ...
        print(
            f"   ✅ Database diagnosis complete - {db_metrics['table_count']} tables, {db_metrics['index_count']} indexes"
        )
    ...
    async def diagnose_security_area(self):
        ...
        print(
            f"   ✅ Security diagnosis complete - Overall score: {security_metrics['compliance_score']}/100"
        )
    ...
    async def diagnose_compliance_area(self):
        ...
        print(
            f"   ✅ Compliance diagnosis complete - FATF: {compliance_metrics['fatf_compliance']}%, GDPR: {compliance_metrics['gdpr_compliance']}%"
        )
    ...
    async def diagnose_performance_area(self):
        ...
        print(
            f"   ✅ Performance diagnosis complete - Scalability: {performance_metrics['scalability_score']}/100"
        )
    ...
    async def analyze_attack_vectors(self):
        ...
        print(
            f"   ✅ Attack vector analysis complete - {sum(1 for v in attack_vectors.values() if v['score'] >= 90)}/6 vectors well-protected"
        )
    ...
    async def analyze_data_flow_vectors(self):
        ...
        print(
            f"   ✅ Data flow analysis complete - Average integrity: {sum(v['integrity'] for v in data_flows.values()) // len(data_flows)}%"
        )
    ...
    async def analyze_integration_vectors(self):
        ...
        print(
            f"   ✅ Integration analysis complete - Average reliability: {sum(v['reliability'] for v in integrations.values()) // len(integrations)}%"
        )
    ...
    async def analyze_scalability_vectors(self):
        ...
        print(
            f"   ✅ Scalability analysis complete - Average efficiency: {sum(v['efficiency'] for v in scalability.values()) // len(scalability)}%"
        )
    ...
    async def assess_technical_dimension(self):
        ...
        print(
            f"   ✅ Technical assessment complete - Architecture maturity: {technical['architecture_maturity']}/100"
        )
    ...
    async def assess_operational_dimension(self):
        ...
        print(
            f"   ✅ Operational assessment complete - SLA achievement: {operational['uptime_sla']}%"
        )
    ...
    async def assess_business_dimension(self):
        ...
        print(
            f"   ✅ Business assessment complete - User satisfaction: {business['user_satisfaction']}%"
        )
    ...
    async def assess_security_dimension(self):
        ...
        print(
            f"   ✅ Security assessment complete - Threat detection: {security['threat_detection_rate']}%"
        )
    ...
    async def collect_performance_metrics(self):
        ...
        metrics = {
            "response_times": {
                "api_p50": round(disk_io_latency_ms / 2, 2)
                if disk_io_latency_ms > 0
                else 15,  # Proxy
                "api_p95": round(disk_io_latency_ms, 2)
                if disk_io_latency_ms > 0
                else 30,
                "api_p99": round(disk_io_latency_ms * 1.5, 2)
                if disk_io_latency_ms > 0
                else 40,
        ...
        print(
            f"   ✅ Performance metrics collected - Disk IO Latency: {metrics['resource_utilization']['disk_io']}ms"
        )
    ...
    async def collect_security_metrics(self):
        ...
        print(
            f"   ✅ Security metrics collected - Fraud detection rate: {metrics['threat_detection']['fraud_detection_rate']}%"
        )
    ...
    async def collect_compliance_metrics(self):
        ...
        print(
            f"   ✅ Compliance metrics collected - Overall adherence: {sum(metrics['regulatory_compliance'].values()) // len(metrics['regulatory_compliance'])}%"
        )
    ...
    async def collect_business_metrics(self):
        ...
        print(
            f"   ✅ Business metrics collected - ROI: {metrics['financial_performance']['roi_percentage']}%"
        )
    ...
    async def collect_quality_metrics(self):
        ...
        if frontend_path.exists():
            ts_files = list(frontend_path.rglob("*.ts")) + list(
                frontend_path.rglob("*.tsx")
            )
            total_lines = 0
        ...
        print(
            f"   ✅ Quality metrics collected - Documentation: {metrics['documentation'].get('doc_files', 0)} files"
        )
    ...
    async def diagnose_code_quality_area(self):
        ...
        quality_metrics["linting"]["eslint"] = (
            "configured" if eslint_config.exists() else "missing"
        )

        # Check for Python formatting
        pyproject = self.repo_root / "pyproject.toml"
        quality_metrics["formatting"]["python"] = (
            "configured" if pyproject.exists() else "missing"
        )

        # Check for TypeScript
        tsconfig = self.repo_root / "frontend" / "tsconfig.json"
        quality_metrics["type_checking"]["typescript"] = (
            "configured" if tsconfig.exists() else "missing"
        )

        self.results["areas"]["code_quality"] = quality_metrics
        print("   ✅ Code quality diagnosis complete")
    ...
    async def diagnose_testing_area(self):
            ...
            for tf in e2e_files:
                try:
                    with open(tf, encoding="utf-8", errors="ignore") as f:
                        content = f.read()
                        total_e2e_tests += content.count("test(") + content.count(
                            "test ("
                        )
                except Exception:
                    pass
        ...
        testing_metrics["e2e_tests"]["playwright"] = (
            "configured" if playwright_config.exists() else "missing"
        )

        self.results["areas"]["testing"] = testing_metrics
        print(
            f"   ✅ Testing diagnosis complete - {testing_metrics['unit_tests'].get('count', 0)} unit tests, {testing_metrics['e2e_tests'].get('count', 0)} E2E tests"
        )
        ...
        if package_json:
            with open(package_json, encoding="utf-8", errors="ignore") as f:
                ...
                dependencies["node_deps"] = {
                    ...
                    "total": len(pkg.get("dependencies", {}))
                    + len(pkg.get("devDependencies", {})),
                }

        self.results["vectors"]["dependencies"] = dependencies
        print(
            f"   ✅ Dependency analysis complete - Python: {dependencies['python_deps'].get('count', 0)}, Node: {dependencies['node_deps'].get('total', 0)}"
        )
    ...
    async def assess_compliance_dimension(self):
        ...
        print(
            f"   ✅ Compliance assessment complete - Overall: {sum(compliance.values()) // len(compliance)}%"
        )
    ...
    async def investigate_critical_areas(self):
        ...
        investigations["security_posture"]["status"] = (
            "review_required" if env_files else "secure"
        )

        # Check for SSOT/Lock files
        lock_files = list(self.repo_root.glob("*.lock")) + list(
            self.repo_root.glob("*.json")
        )
        ssot_files = [
            f
            for f in lock_files
            if "ssot" in f.name.lower() or "lock" in f.name.lower()
        ]
        investigations["reliability_risks"]["ssot_lock_files"] = len(ssot_files)
        investigations["reliability_risks"]["found_files"] = [
            f.name for f in ssot_files
        ]
        investigations["reliability_risks"]["status"] = (
            "good" if ssot_files else "needs_improvement"
        )

        self.results["investigation_details"]["critical_areas"] = investigations
        print("   ✅ Critical area investigation complete")
        ...
        if package_lock.exists():
            self.results["investigation_details"]["package_lock"] = {
                ...
                "last_modified": datetime.fromtimestamp(
                    package_lock.stat().st_mtime
                ).isoformat(),
            }
        else:
            self.results["investigation_details"]["package_lock"] = {"exists": False}

        print("   ✅ Dependency investigation complete")

    async def investigate_configurations(self):
        """Investigate configuration files"""
        print("🔍 Investigating Configurations...")

        configs = {"env_configs": {}, "build_configs": {}, "deployment_configs": {}}

        # Environment configs
        configs["env_configs"]["env_example"] = (
            self.repo_root / ".env.example"
        ).exists() or (self.repo_root / "backend" / ".env.example").exists()
        configs["env_configs"]["env_production"] = (
            self.repo_root / ".env.production"
        ).exists() or (self.repo_root / "backend" / ".env.production").exists()
        configs["env_configs"]["status"] = (
            "configured" if configs["env_configs"]["env_example"] else "missing_example"
        )

        # Build configs
        configs["build_configs"]["dockerfile"] = (
            self.repo_root / "Dockerfile"
        ).exists() or (self.repo_root / "backend" / "Dockerfile").exists()
        configs["build_configs"]["docker_compose"] = (
            self.repo_root / "docker-compose.yml"
        ).exists()
        configs["build_configs"]["vite_config"] = (
            self.repo_root / "frontend" / "vite.config.ts"
        ).exists()
        configs["build_configs"]["containerized"] = (
            configs["build_configs"]["dockerfile"]
            and configs["build_configs"]["docker_compose"]
        )

        self.results["investigation_details"]["configurations"] = configs
        print("   ✅ Configuration investigation complete")
    ...
    async def investigate_documentation(self):
        ...
        # API Docs
        docs_path = self.repo_root / "docs"
        api_docs = list(docs_path.rglob("*api*.md")) + list(
            self.repo_root.rglob("openapi*.json")
        )
        ...
        docs["user_guides"]["locations"] = [
            str(p.relative_to(self.repo_root)) for p in readmes[:5]
        ]  # List top 5
        ...
        # Calculate score
        score = 0
        if docs["api_docs"]["exists"]:
            score += 30
        if docs["user_guides"]["readme_count"] > 5:
            score += 30
        if docs["technical_docs"]["count"] > 10:
            score += 40
    ...
    def calculate_maturity_scores(self):
        ...
        if compliance_dim:
            maturity["compliance_maturity"] = sum(compliance_dim.values()) // len(
                compliance_dim
            )

        self.results["scores"]["maturity"] = maturity
        print(
            f"   ✅ Maturity scoring complete - Average: {sum(maturity.values()) // len(maturity)}%"
        )
    ...
    def generate_executive_summary(self):
        ...
        for area, metrics in self.results["areas"].items():
            if isinstance(metrics, dict):
                score = metrics.get(
                    "compliance_score", metrics.get("scalability_score", 0)
                )
                if score >= 90:
        ...
        for area, metrics in self.results["areas"].items():
            if isinstance(metrics, dict):
                score = metrics.get(
                    "compliance_score", metrics.get("scalability_score", 100)
                )
                if score < 75:
        ...
        print(
            f"   ✅ Executive summary generated - Health: {summary['overall_health']}, Maturity: {summary['maturity_level']}"
        )
    ...
    def calculate_overall_scores(self):
        ...
        for area, metrics in self.results["areas"].items():
            if "services_status" in metrics:
                healthy_services = sum(
                    1
                    for s in metrics["services_status"].values()
                    if "healthy" in str(s)
                )
                area_scores[area] = (
                    healthy_services / len(metrics["services_status"])
                ) * 100
        ...
        scores["areas_overall"] = sum(
            area_scores.get(area, 85) * weight for area, weight in area_weights.items()
        )
        ...
        for vector_type, vectors in self.results["vectors"].items():
            if isinstance(vectors, dict):
                vector_scores[vector_type] = sum(
                    v.get(
                        "score",
                        v.get(
                            "efficiency", v.get("integrity", v.get("reliability", 100))
                        ),
                    )
            ...
            if "compliance_score" in metrics:
                ...
                if isinstance(metrics, dict) and metrics:  # Check if dict is not empty
                    ...
                    dimension_scores[dimension] = (
                        100  # Default if no specific metric or empty dict
                    )

        scores["dimensions_overall"] = sum(dimension_scores.values()) / len(
            dimension_scores
        )
        ...
        print(
            f"   ✅ Overall scoring complete - Platform Score: {scores['platform_overall']:.1f}/100"
        )
        ...
        for area_name, area_metrics in self.results["areas"].items():
            if isinstance(area_metrics, dict):
                ...
                if area_name == "backend" and area_metrics.get("api_endpoints"):
                    ...
                    if (
                        isinstance(endpoint_info, dict)
                        and endpoint_info.get("total_endpoints", 0) == 0
                    ):
                        recommendations.append(
                            {
                                "priority": "medium",
    ...
    def save_markdown_report(self, filename: str):
        ...
        report.append(
            f"- **Platform Score:** {self.results['scores']['platform_overall']:.1f}/100"
        )
        report.append(f"- **Critical Issues:** {exec_summary['critical_issues']}")
        report.append(
            f"- **High Priority Issues:** {exec_summary['high_priority_issues']}"
        )
        report.append(
        ...
        for score_type, score_value in self.results["scores"].items():
            if isinstance(score_value, dict):
                ...
                for key, value in score_value.items():
                    ...
                    report.append(
                        f"- {status} **{key.replace('_', ' ').title()}:** {value:.1f}/100"
                    )
                report.append("")
            else:
                status = (
                    "🟢" if score_value >= 85 else "🟡" if score_value >= 70 else "🔴"
                )
                report.append(
                    f"- {status} **{score_type.replace('_', ' ').title()}:** {score_value:.1f}/100\n"
                )
        ...
        if self.results.get("investigation_details"):
            ...
            for investigation_type, details in self.results[
                "investigation_details"
            ].items():
...
async def main():
    ...
    try:
        ...
        for area, metrics in results["areas"].items():
            if "compliance_score" in metrics:
                ...
                if "services_status" in metrics:
                    ...
                    score = (
                        (healthy / len(metrics["services_status"])) * 100
                        if metrics["services_status"]
                        else 85
                    )
                else:
                    score = 85
        ...
        for vector_type, vectors in results["vectors"].items():
            if vectors:
                avg_score = sum(
                    v.get("score", v.get("efficiency", v.get("reliability", 85)))
                    for v in vectors.values()
                ) / len(vectors)
                status = "🟢" if avg_score >= 85 else "🟡" if avg_score >= 70 else "🔴"
                print(
                    f"   {status} {vector_type.replace('_', ' ').title()}: {avg_score:.1f}/100"
                )
        ...
        for dimension, metrics in results["dimensions"].items():
            if metrics and isinstance(metrics, dict):
                ...
                status = "🟢" if avg_score >= 85 else "🟡" if avg_score >= 70 else "🔴"
                print(
                    f"   {status} {dimension.replace('_', ' ').title()}: {avg_score:.1f}/100"
                )
        ...
        critical_recs = [
            r for r in results["recommendations"] if r.get("priority") == "critical"
        ]
        high_recs = [
            r for r in results["recommendations"] if r.get("priority") == "high"
        ]
        ...
        if critical_recs:
            ...
            for i, rec in enumerate(critical_recs[:5], 1):
                ...
                print(
                    f"      Impact: {rec['impact'].upper()} | Effort: {rec['effort'].upper()}"
                )
